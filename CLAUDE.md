...
**レビューが不要だと判断した場合**(docsのみの変更など): 黙って飛ばさず、
`### Code review skipped: <理由>` という見出しのコメントをPRへ残す。フックはこの形式も
通すが、**理由が無い(見出しだけの)コメントは通さない**——「不要だと判断した事実と理由を
残す」ためのものなので、理由が無ければ記録として意味を成さないため。
