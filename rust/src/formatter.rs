...
//   判断しないと整形が意味を変える(`!(x is none)`→`!x is none`)。改行の扱いと違い、
//   ここは印字側の判断が要る唯一の場所
...
//
// **TS版は`replace`を連ねる多段パス**(`\\`の倍化 → `"` → 改行 → タブ → 最後に`${`)なのに対し、
// こちらは**1文字先読みの単一パス**にしている。等価である理由: 各アームは自分が消費した文字に
// 対応する出力だけを書き、`$`の先読み(`peek`)は`{`を**消費しない**ので、`{`は次の反復で
// 通常どおり処理される。TS版で問題になりうる「前段が書いたバックスラッシュを後段が再走査して
// 二重に処理する」という現象も、前段・後段という段が無いので起きない。
// (`\`+`${1}`のような隣接ケースで両者の出力が一致することを確認済み)
// **エスケープ対象を増やすときは要注意**——多段パスなら自然に噛み合う組み合わせでも、
// 単一パスでは先読みの幅を明示的に広げないと取りこぼしうる(code reviewでの指摘)
