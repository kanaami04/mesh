...
// TS版は107種すべてを説明できるが、Rust版のfull_checkerはまだ36種しか出さない。存在しない
// 検査の説明を並べると「Rust版でもその診断が出る」という誤解を招くので、
// `DiagnosticCode::ALL`(=実装済みの検査)に絞る。したがって:
//   - `mesh explain`(引数無し)の件数行はTS版の107ではなく36になる(意図的な差)
