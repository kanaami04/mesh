    ...
    loop {
        match child.try_wait().expect("子プロセスの状態を取れること") {
            ...
            None if std::time::Instant::now() >= deadline => {
                let _ = child.kill();
                let _ = child.wait();
                return RunOutcome { stderr: String::new(), timed_out: true };
            }
            None => std::thread::sleep(Duration::from_millis(20)),
        }
    }
    let out = child.wait_with_output().expect("出力を回収できること");
    RunOutcome { stderr: String::from_utf8_lossy(&out.stderr).into_owned(), timed_out: false }
}
...
fn 診断ゼロで実行できたプログラムはコンパイラのバグで落ちない() {
    ...
    for f in &files {
        ...
        if outcome.timed_out {
            broken.push(format!("{rel} — 30秒で終わらなかった(停止しないプログラムはコーパスに置かない)"));
            continue;
        }
