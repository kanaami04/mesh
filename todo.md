...
- [ ] **Rust移植の開始** — 2026-07-21着手(kanayamaと討議のうえ。進め方は今まで通り
  ...
  - [ ] **フルchecker(診断)+CLI移植** — 2026-07-24提起・方針合意。**milestone 22(第一歩)は
        ...
        - [x] **milestone 43: `type-alias-cycle`(裸のunion循環)**
              ...
              - 診断コード55→56種。新規テスト4件(566→570件)。clippy(`-D warnings`)クリーン。
              - **検証**(TS版と突き合わせ): 循環の形を13通り(2つ巴/3つ巴/自己参照×union・
                ...
                全体では全`.mesh` + プローブ計133ファイルで、差が出るのは他の未移植機能
                (ジェネリック呼び出しの引数照合・`cannot-infer-type`の適用範囲)が絡む
                2ファイルのみ、**Rust側だけに出る診断は0件**。examples全22件の`mesh run`も一致。
