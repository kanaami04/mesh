...
- [ ] **Rust移植の開始** — 2026-07-21着手(kanayamaと討議のうえ。進め方は今まで通り
  ...
  - [x] **checker+codegen milestone 14(比較/論理/等価演算子の妥当性検査)**
        ✅ 2026-07-24実装。milestone 13完了後、kanayamaから「妥当性検査の部分
        全てやってしまいましょう」と依頼を受け、milestone 13で意図的にスコープ外と
        していた残り3カテゴリ(`&&`/`||`の`not-bool`・`==`/`!=`の`use-is-none`/
        `incomparable-types`・`< <= > >=`の`incomparable-types`)をまとめて実装。
        - **TS版の該当ロジック**(`src/checker/expressions.ts:555-608`、milestone 13の
          調査時に既に読んでいた箇所): `&&`/`||`は左右それぞれのオペランドが
          bool(またはANY)であることを要求し、それぞれ**自分自身の位置**
          (`expr.left.pos`/`expr.right.pos`、二項式全体の位置ではない)でエラーに
          する。`==`/`!=`はリテラル`none`との比較(`x == none`)を、型を問わず
          常にErrにする(P1: narrowingが効く`is none`に一本化する言語ルールそのもの
          ——構文上`Expr::None`が直接現れるかどうかだけを見る、型推論は不要)。
          それ以外の`==`/`!=`は双方向assignable(どちらかの型がもう一方に代入
          できれば比較可能)でなければErr。`< <= > >=`は両方numeric・両方stringy・
          どちらかANYのいずれかでなければErr。
        - `checker.rs`: `infer_binary`の`match`を分割し、新設`check_logical_op`
          (`&&`/`||`)・`check_equality_op`(`==`/`!=`)・`check_comparison_op`
          (`< <= > >=`)へ振り分ける。`check_equality_op`は`Expr::None`の構文的
          検出が必要なため`ctx`+`Expr`を直接受け取り、`check_comparison_op`は
          既存の`check_arith_op`と同じく解決済み`Type`のみを受け取る(値そのものは
          不要なため)。型互換性は既存の`types::assignable`/`is_numeric`/
          `is_stringy`をそのまま再利用——新しい代入規則は追加していない。
        - `codegen.rs`は変更不要——milestone 13で`Expr::Binary`/`gen_compound_value`
          両方の`infer_binary`呼び出しに`?`が既に付いているため、新しいErrは
          自動的に伝播する。
        - 新規ユニットテスト`checker.rs`5件(論理演算子の非bool検査・左右どちらの
          エラーも検出できること・等価演算子のリテラルnone検査・双方向assignable
          検査・順序比較の数値/文字列/ANY判定)+`codegen.rs`4件(非bool論理演算子・
          リテラルnoneとの等価比較・比較不能な型同士の等価/順序比較・正常な
          論理/等価/順序比較が今まで通りコンパイルできること)。322→330件、全件
          パス。`cargo clippy --all-targets -- -D warnings`クリーン。既存の全
          example(22本、`&&`/`||`/`==`/`!=`/`< <= > >=`を最も広範囲に使う演算子
          カテゴリ全てを新規検証対象にしたため今までで最も回帰リスクが高かったが、
          全てbyte-for-byte一致で回帰無しを確認——理由はmilestone 12/13と同じ
          (既存の全exampleは既にTS版のこれらの検査を通過しているプログラムの
          ため)。`x && true`(非bool)・`x == none`(リテラルnone比較)・
          `User == 5`(比較不能な等価)・`User < User`(比較不能な順序比較)の
          4パターンをRust版・TS版両方でコンパイルし、同じ理由(`not-bool`/
  ...
  - [ ] **フルchecker(診断)+CLI移植** — 2026-07-24提起・方針合意。**milestone 22(第一歩)は
        ...
        - [x] **milestone 35: Rust CLIの`run`/`build` + full_checkerのゲート統合**
              ...
              full_checkerもどのコマンドからも使われていなかった。
              - `main.rs`をTS版`cli.ts`と同じサブコマンド構成(`run`/`build`/`check`/`ast`)へ
              ...
              - **code reviewで発見・即修正した2件**(いずれもTS版と突き合わせて再現確認):
                ...
                (TS版は検出する)——全モジュールを1本ずつ検査する形にして解消。
                (3)**`--json`が構文エラーのときだけ素のテキストを出していた**——TS版
                `compileModules`はパースエラーを型検査の診断と同じ配列へ畳み込むのでJSONの
                ままになる。`--json`はREADME/requirements.mdが「エージェントの自己修正ループの
                前提」として明記している機械可読の契約で、しかも構文エラーはそのループで最も
                頻繁に踏む状態なので実害がある。パース失敗を表示せず呼び出し元へ返す形
                (`LoadFailure`)に変え、`--json`のときはJSONへ載せるようにした(モジュール発見の
                失敗はTS版も素のエラー文なので揃える)。回帰テスト(multi-fileのゲート・
                stderrへの出力・構文エラーのJSON)を追加、10→12件。
              - **次段階**: `fmt`/`test`/`card`/`explain`の移植、full_checkerの複数ファイル対応
                (ゲートを全モジュールへ広げる)。
        - [x] **milestone 36: `mesh fmt`(フォーマッタ)の移植**
              ✅ 2026-07-25実装。「TS実装の撤去条件」の(1)の続き。TS版`formatter.ts`(486行)を
              `rust/src/formatter.rs`へ移植し、CLIに`mesh fmt <file> [-w]`を追加した。
              - **前提としてAST/parserの拡張が必要だった**(ここが本題): フォーマッタは
                (a)`Program.comments`——lexerは既にコメントを退避していたがparserが捨てていた、
                (b)`multiline`フラグ——「元ソースで複数行だったか」をBlock/ArrayLit/MapLit/
                StructLit/Call/Unionの6箇所に記録する、の2つを必要とする。gofmt方式で
                「幅による自動折り返しをしない」ため、改行の判断はparserが記録した事実だけに
                従う設計(TS版と同じ)。フィールド追加でパターンマッチ側も`..`を足して回った。
              - 印字はTS版の構造をそのまま移植: `Printer`(行バッファ+コメントの位置ベース
                再割り当て)・`print_program`/`print_type_decl`/`print_fn_decl`/`print_stmt`/
                `print_expr`/`print_type_node`/`print_braced_list`/`escape_string`。
                **複数行の呼び出し引数だけ末尾カンマを付ける**というASI対策(TS版が実際に踏んだ
                バグ——素朴に改行すると閉じ括弧の前でセミコロンが挿入され再パースで壊れる)も
                含めて移植。`${`のエスケープ(整形でプログラムの意味が変わってはいけない)も同様。
              - `mesh fmt`は**モジュールを辿らず対象ファイル1本だけ**を読む(整形はファイル単位の
                操作でimport解決が要らない。TS版も同じ)。`-w`で書き戻し、構文エラーは
                ソース行つきでstderrへ出して**書き戻さない**。
              - **検証**: 全example 26本(パッケージのサブディレクトリ含む)で`mesh fmt`の出力が
                TS版と**バイト一致**。整形の冪等性(fmt(fmt(x)) == fmt(x))も26本すべてで確認。
                整形後のソースを実行しても**出力は変わらない**ことを確認(空行が減るぶん
                panicメッセージの行番号はずれる——`defer_panic.mesh`で9:8→8:8。整形は行番号を
                保存しない操作なので想定どおり)。formatterの単体テスト10件+CLIテスト3件
                (stdout出力・`-w`の書き戻し・構文エラー時に書き潰さない)を追加。
                495→505件+CLI 12→15件、`cargo clippy --all-targets -- -D warnings`クリーン。
              - **既知の限界(TS版と共通)**: 空行を保持しない(トップレベル宣言間は常に1行、
                文の間は常に0行に正規化)。既存example 26本中22本が整形で変化する。
                内訳は(すべてTS版と同じ出力): 空行の除去のみが14本、それ以外の正規化を含むものが
                12本——1行の`if`/`match`をブロックへ展開・struct literal内側の空白除去
                (`Tree{ kind: ... }`→`Tree{kind: ...}`)・trailingコメント前を空白2つに統一・
                `?`の前の空白除去・複数行呼び出しの末尾カンマ付与。**当初「差分はすべて関数内の
                空行が消えるだけ」と書いていたが不正確だった**(code reviewで指摘・訂正)。
              - **code reviewで発見・即修正した1件**(両CLIで再現確認): `try_print_stmt_inline`に
                `defer`を入れていたため、`fn() { defer cleanup() }`のような**deferを含む1行
                クロージャがTS版と食い違っていた**(TS版`tryPrintStmtInline`には`deferStmt`の
                caseが無く常に複数行へ展開される)。この関数は「無名関数の本体を1行に保てるか」の
                判定を兼ねているのが理由。TS版と同じくinline判定から外し、通常の文としての印字は
                `print_stmt`が直接行う形にした。全example 26本のバイト一致では検出できなかった
                (この形のコードがexampleに無かった)。回帰テスト追加。
                あわせて過去の実バグ2件(`json struct`のキーワード落ち・`defer`文の消失)の
        ...
        - [x] **milestone 44: `defer-requires-call`**
              ✅ 2026-07-26実装。小さく独立した1診断。
              - `defer`の後ろが**呼び出しでなければ**弾く(`defer 5` / `defer work` /
                `defer 1 + 2` / `defer b.n` / `defer xs[0]` / `defer <-ch` / `defer spawn f()`)。
                括弧付き(`defer (f(1))`)はparserが外すのでCallのまま通る。
              - **順序が要**: TS版`statements.ts`と同じく「呼び出しかどうか」を**先に**見て、
                そうでなければそこで打ち切る。呼び出しでない式をそのまま検査しても位置のずれた
                別の診断になるだけで、本来報告すべき「呼び出しじゃない」が消える
                (TS版のコメントにも同じ理由が書いてある。`defer 5`は式として検査しても
                何のエラーにもならない)。呼び出しなら中身(引数)も従来どおり検査する。
              - 診断コード56→57種。新規テスト2件(571→573件)。clippy(`-D warnings`)クリーン。
              - **検証**(TS版と突き合わせ): defer固有のプローブ19本(正常系2・非呼び出し8・
                メソッド内/無名関数内/matchのアーム後/引数が型不一致/**壊れた型宣言と同居**/
                入れ子ブロック/複数のdeferが並ぶ形など)で**すべて出力・終了コードが一致**。
                全体では全`.mesh` + プローブ計155ファイルで、差が出るのは既知の未移植診断
                (ジェネリック呼び出しの引数照合・`cannot-infer-type`の適用範囲・
                型宣言の`already-declared`)が絡む5ファイルのみ、**Rust側だけに出る診断は0件**。
                examples全22件の`mesh run`も一致。
        - [x] **milestone 45: 関数型のモデル化 + `not-callable`**
              ✅ 2026-07-26実装。milestone 44のcode reviewで見つかった穴(`chan<fn()>`への
              代入互換性・受信した関数値の呼び出し)の根を断つ回。
              - **`resolve_type_ann`が関数型を解決するようになった**(それまでは一律ANY)。
                unionのときと同じ規則——**引数か戻り値が1つでもANYへ縮退したら全体をANY**に
                するので、「中身を完全に解決できた関数型だけが関数型として残る」。
                `is_fully_modeled`も関数型を(中身が解決できていれば)受け入れる側に変えた。
              - **`assignable_checked`を新設**して、full_checker側の型突き合わせを1箇所に集約した
                (26箇所の`types::assignable`呼び出しを差し替え)。**どちらかにANYが潜んでいたら
                比較しない**——縮退した型と完全解決された型を突き合わせると誤検知になる、という
                この移植で何度も踏んだ穴への一般的な門番。関数型のモデル化で「中にANYが潜みうる型」が
                増えたため、個別ガードではなくここへ集約する形にした。
              - `not-callable`(関数でない値の呼び出し)を追加。ANYは免除する。
              - 関数値フィールドの呼び出し(`h.cb(1)`)も引数照合されるようになった
                (TS版`memberFieldType`→`checkCallOfValue`と同じ経路)。
              - **既存テスト2件が「縮退時代の期待値」だったので更新**した——どちらもTS版に
                問い合わせて、新しい挙動がTS版と一致することを確認してから書き換えている:
                (1)同じ形のstructメンバーを持つunionは`discriminated-union-ambiguous`を出す
                (以前は絞り切れず黙っていた=検出漏れ)、(2)`fn(int[]) int[]`のフィールドへ
                関数の配列を入れる例は`type-mismatch`+`builtin-arg-type`が出る。
                **モデル化が進むと「縮退ゆえに黙っていた」テストは期待値が変わる**——
                このパターンはmilestone 33/34(コレクション)でも起きており、今後も起きる。
              - **ついでに見つかった既存の乖離を1件修正**: 型注釈つき宣言(`x: int = "s"`)の
                type-mismatchが、TS版と**位置も文言も違って**いた(TS版は値の位置に
                `cannot use X as Y`、Rust版は文頭に独自の文言)。関数型のプローブを書いていて
                気づいた——型注釈つき宣言に値の型不一致を置いたプローブがそれまで無かった。
              - 診断コード57→58種。新規テスト3件+既存2件の更新(573→576件)。clippyクリーン。
              - **検証**(TS版と突き合わせ): 関数型プローブ13本(非呼び出し3種・関数型注釈の
                引数/フィールド/宣言/チャネル要素での不一致・正しい形・関数値フィールドの
                呼び出し・型注釈つき宣言の位置)で一致。全体では全`.mesh` + プローブ計169ファイルで、
                差が出るのは既知の未移植診断が絡む5ファイルのみ、**Rust側だけに出る診断は0件**。
                examples全22件の`mesh run`も一致。
        ...
        - [x] **milestone 54: pkg跨ぎのstructをフィールド型に持つ値の型検査**
              ✅ 2026-07-27実装。**新しい診断コードは足していない**(69種のまま)——
              milestone 51のcode reviewで実測して記録した検出漏れのうち、一番効くものを塞いだ回。
              - **原因**: `has_unregistered_struct`が`lookup_struct`(パッケージ内の**素の名前**の表)
                しか引かないため、フィールドの宣言型が他パッケージのstruct(`lib.Item`)だと
                必ず「未登録」と判定され、そのフィールドの値の型検査が**丸ごと免除**されていた。
                milestone 51でpkg修飾型がレジストリに載るようになったので引けるようになった。
              - **実測して1回直した**: 最初`is_package_alias`も条件に入れたが、それだと
                `import "lib2"`だけしている側で`lib2.Container.item`(型は`lib.Item`)の検査が
                また免除される。**この名前は型注釈に書かれた参照ではなく、既に解決済みの型が
                持っている名前**で、現在のパッケージのimportとは無関係——import宣言を経由しない
                参照を弾く不変条件(milestone 6)は`resolve_type_ann`側で担保されている。
              - **検証**: プローブ8本(pkg修飾structリテラルのフィールド・同一パッケージの
        ...
        - [x] **milestone 56: unionのモデル化(`narrow-required` + 関数型シグネチャ)**
              ✅ 2026-07-27実装。**新しい診断コードは足していない**(69種のまま)。
              milestone 55で取り下げた`is_fully_modeled`のunion緩和を、**正しい軸**で入れ直した回。
              残っていた候補2つ((c)戻り値がunionの関数型 / unionフィールドのモデル化)が
              **1つの変更で両方片付いた**。
              - **軸は「自己参照かどうか」ではなく「空フィールドの殻structを含むか」だった**。
                milestone 55では自己参照を除こうとして失敗した——`Node | none`も`json.Value`も
                どちらも自己参照だから。実際の違いは`json.Value`が再帰位置を**名前だけの殻**
                (`struct_ty("json.Value", vec![], false)`、milestone 9)に留めていること。
                殻を含むunionだけ突き合わせから外せば、両方が正しく扱える。
              - **フィールドパスの絞り込み(TS版`stablePath`)が必要になった**。unionが
                突き合わせ対象になった結果、`if b.next is Node { b.next.n }`が
                **絞り込み済みなのに`narrow-required`**という誤検知になった(手順2'で検出)。
                `a.b.c`という綴りをスコープのキーにしてTS版と同じ形で解決。
                **根が`mut`なら絞り込まない**のもTS版と揃えた(再代入で中身が変わりうる)。
              - `http.listen`のコールバックのシグネチャ検査も同時に効くようになった
                (戻り値が`none | error`のためシグネチャ全体がANYへ落ちていた)。
              - **検証**: プローブ11本(絞り込み後のパスアクセス・入れ子パス・`mut`・
                呼び出し結果のパス・else節・match・`json.Value`の1階層destructure・
        ...
        - [x] **性質検査 第1弾: 整形が意味を変えないことをコーパス全体で検査する**
              ✅ 2026-07-28実装。TS撤去で失ったのは「テストの本数」ではなく**オラクル**
              (新しい入力にも答えを出せる相手)だという整理から、**答えを知らなくても
              正誤を判定できる**検査を足し始めた。考え方と注意点は
              `docs/handoff.md`「性質検査(オラクルの代わり)」節が一次情報源。
              - **試作で効きを実測してから入れた**。milestone 65の修正前コミット(`4016215`)に
                「意味の等価な条件の書き換えで診断は変わらない」を当てると**40件の破れ**を検出。
                当時この検査があれば、あの誤検知5種はmainへ出荷される前に止まっていた。
                期待値ファイルは1つも要らない(変種同士を突き合わせるだけ)。
              - **その試作が別の実バグを見つけた**——`mesh fmt`が意味に効く括弧を落としていた。
                `!(x is none)`→`!x is none`(`!`がintに掛かる)、`(a + b) * c`→`a + b * c`、
                `-(a + b)`→`-a + b`、`(t || u) && t`→`t || u && t`。`mesh fmt -w`は
                ソースを書き戻すので、意味が変わったコードがそのまま残る形だった。
              - 原因: ASTに`Expr::Paren`は無く(パーサが括弧を捨てる)、印字側が優先順位から
                括弧を**再導出**する必要があるのに`print_expr`が子をそのまま連結していた。
                優先順位表を`parser::precedence`に一本化し(2箇所に持つとずれる)、
                formatterは`prec_of`/`print_operand`でそこから導出する。`prec_of`は
                網羅的にmatchしてあり、新しいExpr変種を足すと判断を強制される。
                `- -x`だけは優先順位では防げない(`--`とlexされる)ので括弧で切る。
              - **なぜ既存の性質テストで見つからなかったか**が本題。`fmt_corpus.rs`には
                既に「整形はべき等で意味を変えない」があったが、意味の保存を**実行して
                標準出力を比べる**方法で見ていたため、**走らないプログラム(診断を出させる
                ための`tests/parity/`132ケース)には原理的に効かなかった**。不具合は
                ちょうどそこに住んでいた。しかも**べき等性は保たれたまま意味だけ壊れる**
                (`!x is none`は再整形しても変わらない)ので、べき等性の検査も素通りしていた。
                判定を「実行結果の一致」から「**診断の一致**」へ広げ、対象が24本→156本に。
                JSランタイム非依存で走るのも利点。**修正を外すと落ちることを確認済み**。
              - **副産物の事故**: 2つのテストが検査対象の隣に一時ファイルを置くため、
                cargoの並行実行で互いの一時ファイルを`read_dir`が拾い、偽の失敗になった。
                `__`始まりを除外して解消(異常終了で残った物を拾わない効果もある)。
              - あわせて`docs/features.md`のdrift 3件を修正(`mesh fmt`が🔜のまま/
                「テストは`bun test`」がTS撤去後も残存/「複数エラーの一括報告」が🔜のまま
                ——同じ表の別の行では✅と書いてある)。
              - 検証: テスト676件、clippyクリーン、`mise run parity` 156件差0、
                `mise run sweep` 120件差0、`run-examples` 24本、`mise run drift` 候補0。
              - **次の候補**(未着手): 変数名の一貫リネームで診断不変 / コメント・空行の挿入で
                診断不変 / 診断ゼロで通ったプログラムは実行してpanicしない /
                どんな壊れた入力でもコンパイラがpanicしない。試作した「等価な条件の書き換え」
                (性質A)も`scripts/`へ昇格させる候補だが、等価集合の吟味が要る
                (`x is int || 1 == 2`が絞り込まないのは仕様の可能性が高い)
        - [x] **`__`接頭辞を予約する(性質検査 第4弾の準備で出た言語の穴2件)**
              ✅ 2026-07-29実装。詳細は `docs/handoff.md`「`__`接頭辞を予約した」節。
              **リネームの性質検査そのものは未着手**——生成器を書く前に危ない名前を手で
              試したら、言語レベルの穴が2つ出たので先に塞いだ回。
              - **穴1**: `__m := 100` のあと `match v { int => __m }` が100ではなく
                **subjectの値**を返す(matchがsubjectを`(async (__m) => ...)`のIIFE引数で
                渡すため)。**checkもbuildも通って実行結果だけが誤る**、いちばん悪い形。
                `__idx := 42` はランタイム関数を隠して`42 is not a function`。
                **どちらもTS版オラクルで同じ結果**——移植の問題ではなく最初からある言語の穴。
              - **kanayamaと討議して「`__`で始まる名前の宣言を禁止」を採用**。Meshでは`__`に
                意味が無く(可視性は`export`で決まる)ユーザーが失うものは無い一方、コンパイラは
                「一時変数を増やすたびに誤コンパイルの種が増える」状態を終わらせられる。
                フィールド名`__proto__`の個別拒否を一般化する形でもある。
                実装は`declare()`の1箇所+**診断は既存の`reserved-word`を流用**(新コード無し)。
              - **parityを回して初めて誤検知10件に気づいた**。`json struct`の合成デコーダは
                `__ef_*`等を10種類使っており、それは`__`を予約した目的そのもの。合成物は
                「以降のcheck/codegenを無改造で流用する」ため普通の`FnDecl`として積まれ
                **ASTから区別できない**ので、`check_package`がjson structから決まる名前
                (`decode{Name}`/`encode{Name}`)で判定し本体検査中だけ免除する形にした。
              - **穴2(同じ回で発見)**: `_ = expr` が壊れたJSを生成していた。`__`の予約が
                ブランク識別子を巻き込んでいないことを確認していて見つかった。`_`は宣言されない
                のにcodegenが`_ = ...`と出し、実行時`_ is not defined`——**`build ⇒ run`の破れ**
                (第3弾が`internal error:`として正しく分類した)。右辺を式文として出し
                **副作用を保ったまま値だけ捨てる**形に。経路は文とforのinit/postの**2つ**で両方直した。
              - カード(`rust/embedded/card.ts`)のBindings節と「よくあるエラー→直し方」にも追記。
              - 検証: cargo test 692件 / clippy 0 / parity 170件差0 / sweep 126件差0 /
                run-examples 24本 / json関連exampleの実行確認。
        - [x] **性質検査 第3弾: `build ⇒ run`(診断ゼロでビルドできたプログラムを実行しても
              コンパイラのバグでは落ちない)**
              ✅ 2026-07-29実装(`rust/tests/build_implies_run.rs`)。
              詳細は `docs/handoff.md`「性質検査 第3弾」節が一次情報源。
              - **なぜ要るか**: `check_implies_build`は**コード生成が成功すること**までしか
                見ない。生成JSが実行できるかは別で、実際にそこに穴があった——milestone 68の
                code reviewが**人力で**見つけた「非mainパッケージのconst参照が`check`も`build`も
                通って実行時に`x is not defined`」がそれ。
              - **判定の言い換えが要点**。「実行して期待どおりの結果を出す」はそのままだと
                「期待どおり」の答えが必要でオラクル非依存にならない(記録にすれば`--update`の
                抜け道も戻る)。そこで**`__Panic`(仕様内の停止)と素のJSエラー(コンパイラの
                バグ)を区別**し、後者が出ないことだけを見る形にした。標準ライブラリの失敗経路は
                すべて`error`値へ変換しているので、`__Panic`以外が`__panic`へ届くのは
                生成JSが壊れているときだけ。
              - **ランタイムの表示を分けた**(`rust/embedded/runtime.ts`の`__panic`):
                仕様内は`panic:`のまま、それ以外は`internal error: ...`+「これはMeshコンパイラの
                バグです」の案内。`panic:`に丸めていた頃は、コード生成のバグをユーザーが
                自分のプログラムの問題として調べる形になっていた(kanayamaと討議のうえ採用)。
              - 現在**58件を実行して0件**(残り110件は診断が出るので性質の前提外)。
              - **教訓1: コーパスに形を置くまでが仕事**。導入直後にcodegenを壊しても落ちなかった
                ——該当の形がコーパスに無かったため。`tests/parity/pkg-const-runtime/`を足して
                から壊すと`internal error: maybe is not defined`で落ちた。
              - **教訓2: 土台の検査は「印が出る方向」を測る**。判定の土台を守るテストを
                「仕様内のpanicに`internal error:`が付かないこと」だけで書いたら、印を消しても
                **両方のテストが素通り**した。`__panic`へ直接`new Error`/`new __Panic`を渡して
                両方向を固定した(PRELUDEは`--emit-js`の出力から取る=出荷する本物を測る)。
              - 検証: cargo test 689件 / clippy 0 / parity 168件差0 / sweep 126件差0 /
                run-examples 24本 / `mesh test`の隔離も文言そのまま。壊して落ちることを2方向で実測。
        ...
        - [x] **オラクル差分ハンター(`mise run oracle-hunt`)— loop engineering 段階2の中身**
              ...
              詳細は `docs/handoff.md`「オラクル差分ハンター」節。
              - **設計が当初から変わった**。段階2は「性質検査を定期実行する」つもりだったが、
                性質検査は「答えを知らなくても判定できる」ための工夫であって、
                **オラクルが動くならそもそも直接聞ける**。1件70ms、300件で20秒と十分に安い。
                記録を持たないので`--update`に相当する抜け道が構造的に存在しない。
              - **導入時点の結果: 2,500件超・シード7種・差0件**(位置込みで比較しても0件)。
                **撤去条件(5)=両実装の並走確認を、凍結コーパスとは独立に再検証した**ことになる。
              - **語彙を広げるのが主な保守作業**。初版はnarrowingしか作らず800件で0件だった
                ——**直した直後の領域だけを探索しても意味がない**、というのが最初の教訓。
                その後 handoff が繰り返し記録している「実装者の検証を通り抜けた不具合は
                **すべて入力そのものが壊れているケース**だった」に従って`gen_broken`
                (未知の型名 / 同名の宣言 / 引数不足 / 値にならない式 / 絞り込み前のunion使用 等
                20形)を足し、重みも2倍にした。**きれいなプログラムだけ作っても踏めない**。
              - **比較の強さも実測で決めた**。診断コードの集合だけだと弱いので、
                位置(行:桁)込みで250件試して差が出ないことを確認したうえで厳しい側を採った。
              - シードを必ず出力する(再現できなければ調査に入れない)/ 差が出た`.mesh`は
                捨てずに残す / パースできない生成物(SKIP)があれば失敗する、の3点は
                `sweep.sh`の設計をそのまま踏襲。
              - **無人で回すときの自衛を追加**(2026-07-29)。このスクリプトは**バイナリを
                読むだけでリビルドしない**ので、編集途中の作業ツリーで走ると
                「そのとき たまたま`target/debug`にあったバイナリ」を正解として扱う。
                自走ループから無人で回すと**編集途中の状態を差分として報告する**ことになる。
                実行前に「`rust/`か`scripts/`が汚れていないか」「バイナリがソースより
                古くないか」を確かめ、当てはまれば**見送る**(`ORACLE_HUNT_FORCE=1`で上書き可)。
                **失敗ではなくスキップ(exit 3)にするのが要点**——編集中に「差が出た」と
                騒がれても意味がないし、「差0件」も嘘になる。呼び出し側が
                **「見送った」と「差が無かった」を区別できる**ようにした。
              - **このループには期限がある**(2026-07-29、未着手事項として記録)。
                オラクルはTS版が知っている言語しか判定できないので、**本実装(TS版に
                無かった新機能)を実装した瞬間に信号が死ぬ**。性質検査(fmt_corpus/
                check_implies_build)は「答えを知らなくても判定できる」設計のおかげで
                期限が無く、新機能もコーパスに1件足すだけで自動的に守られる——今日の
                資産のうち一番残る部分。切り替えの合図・そのときやること(oracle-huntの
                語彙凍結/ループの中身を性質検査ベースのファジングへ/カード実証実験の開始)は
                `docs/handoff.md`「loop engineering の見取り図が変わる時期」節に整理した。
              - **0件が続くこと自体は失敗ではないが、読まれなくなるのが失敗**。
                0件が続いたら「語彙が狭いのでは」と疑って`FAMILIES`へ族を足す。
              - **code review が「無人運転の前提」を4つ壊してくれた**。どれも
                「壊れた環境が、コンパイラの退行そっくりに見える」形だった:
                引数の検証(生成器が落ちても素通りして**偽の成功**)/ キャッシュの完了印
                (`bun install`が失敗した中途半端なキャッシュを再利用して**13件の偽DIFF**)/
                `bun`の確認をキャッシュの有無に関わらず行う(`command not found`が
                **本物の差分と見分けがつかない**)/ **スモークテスト**(既知の入力に既知の診断が
                返ることを比較の前に確かめる)。**自走させる直前に見つかったのが幸運だった**。
              - **「0件でした」という報告ほど検証が要る**。何も見つからなかったのか、
                **見る目が無かったのか**を区別できない道具は、無人で回すと嘘をつく。
                レビューでは実際に道具を壊して(Rust側・TS側の両方にダミー出力を注入して)
                `DIFF`が出ることまで確かめてもらった。
              - あわせて SKIP 判定の穴も修正。`syntax-error`だけを見ており、
                `unexpected-character`等**字句解析レベルの4コードを取りこぼしていた**。
                いまの生成器は字句エラーを作らないので実害は無かったが、
                **語彙を広げると踏む**(それが主な保守作業だと自分で書いた直後だった)。
        - [x] **codegen側にも `collect_facts` を移植する(絞り込みと無効化をセットで)**
              ...
              - **code review が言語レベルの不健全性を1件見つけた**(このPRの新規バグでは
                ないが、このPRで**顕在化**した)。structは参照値なので、絞り込んだフィールドを
                **関数呼び出し越しに**書き換えられる——`if o.v is int { reset(o); print(o.v + 1) }`
                (`reset`が`o.v`をnoneにする)は `check` も `build` も通り、実行すると
                `null + 1 === 1` で**静かに `1` を出す**。
                **TS版オラクルに同じ入力を流すとまったく同じ結果**なので、移植としては忠実。
                直すなら「呼び出しをまたいだら絞り込みを捨てる」という言語設計の判断が要る
                (実用上かなり厳しくなる)ので、このPRでは記録に留めた。
                プローブは `tests/parity/narrow-alias-through-call/`。
                **#109 との違いは決定的**: あちらは TS版が `invalidatePath` を持っていたので
                **オラクルと食い違う独自のバグ**だった。今回はオラクルの挙動を忠実に再現している。
                「うるさい失敗→静かに誤った出力」という表面は同じでも、**オラクルと一致するか
                しないかで判断が変わる**——これは今日いちばん実用的な判断基準になった。
              - **`check_implies_build` ではこの形を捕まえられない**(checkもbuildも通るため)。
                「ビルドできたプログラムは実行して期待どおりの結果を出す」という次の性質が
                要る具体的な理由が揃った(`docs/handoff.md`「次の候補」に反映済み)。
              ...
              - 検証: テスト685件、clippyクリーン、parity 165件差0、sweep 126件差0、
        ...
        - [x] **`invalidatePath` の移植(絞り込んだパスへ代入したら事実を捨てる)**
              ...
              未移植で、`if o.inner.v is int { o.inner.v = none; print(o.inner.v + 1) }` が
              **診断ゼロ**になっていた(検出漏れ)。`docs/features.md` が「フィールドへの代入は
              古い絞り込みを無効化してから型検査するので健全性は保つ」と書いている安全性が
              実装されていなかった。
              - **オラクルは消えたがTS版のソースは git 履歴に残っている**、というのが今回の
                一番の収穫。`git show cd7273a^:src/checker/narrowing.ts` で実装を、
                `statements.ts` で呼び出し順序を直接読めた。**着手前は「複合代入で誤検知に
                なるのでは」という設計判断を抱えていたが、TS版のコメントに答えが書いてあり
                判断そのものが不要になった**。
                さらにレビューの過程で、**読めるだけでなく動かせる**ことが分かった——
                `src/`を丸ごと復元すれば `bun src/cli.ts check` が当時のまま走り、
                **オラクルに直接問い合わせられる**(実測で確認。手順は `docs/handoff.md`
                「TS版のソースは git 履歴に残っている」節)。これで「Rust版が保守的すぎるのでは」
                という疑い(クロージャ内の代入)を、TS版も同じ診断を出すことを実測して晴らせた。
                **移植漏れを埋めるときの既定手順にする価値がある**。
              - 仕様(TS版どおり): 消すのは**代入先そのものとその子パスだけ**(祖先は残す。
                中間パスの型自体は変わらないため)/ **`.`を含むキーだけ**消す(通常の変数束縛と
                絞り込み事実が同じ表に同居しているため)/ 全スコープを走査する/ 無効化は
                **代入先の型を見る前**(順序が逆だと代入先の型が絞り込み後になり、宣言型には
                代入できるはずの値が弾かれる)/ 右辺は先に全部推論する/ `_`ターゲットは飛ばす。
              - **記録の差分は1件だけ**——`tests/parity/narrow-then-assign-field/` が
                診断ゼロから `invalid-operation` になった。他は無変更で、狙った検出漏れ
                だけが埋まったことの証拠になっている。誤検知の探索は独立したレビューで
                match/selectアーム・クロージャ・`spawn`・シャドーイング・パッケージ跨ぎ・
                多重代入まで当たって**ゼロ**だった。
              - **副産物1: `KNOWN_VIOLATIONS` の腐り検出の穴を塞いだ**。登録ケースが
                診断を出すようになると性質の前提から外れて黙って対象外へ落ち、リストに
                残ったまま誰も気づけない状態だった(今回まさに発生)。リスト整合性テストに
                「登録ケースは`check`が黙っていること」を追加し、両方向とも落ちることを確認。
              - **副産物2: レビューで既存の検出漏れを2件発見**(いずれもこの変更とは無関係)。
                - **フィールド代入の値の型が検査されない**。`check_assign_target`の
                  `Expr::Member`分岐が`infer_expr`を呼ぶだけなので、`t.n = "s"` も
                  `t.n += "s"` も診断ゼロ。**`mesh build`だけが落ちる**ので
                  `check_implies_build`の不変条件の破れでもある。narrowingとは無関係で
                  素の`int`フィールドでも起きる。プローブは
                  `tests/parity/member-assign-unchecked/`(TS版オラクルへ問い合わせた
                  期待値つき)、`KNOWN_VIOLATIONS`へ登録済み
                - **多重代入の個数不一致に`argument-count`が出ない**。TS版は出す。
                  `full_checker.rs`に代入の個数照合ロジックがそもそも無い
                - 当初コメントに「複合代入は算術検査を通るのでTS版と同じく
                  `invalid-operation`になる」と書いていたが**実態と逆**だった
                  (2エージェントが独立指摘)。実測を自分で見ていたのに逆を書いていた形で、
                  出荷前に修正済み
              - **これでcodegen側のフィールドパス絞り込みに安全に再挑戦できる**
                (PR #109 で取り下げたもの)。checkが先に止めるのでミスコンパイルにならない。
              - 検証: テスト679件、clippyクリーン、parity 159件差0、sweep 126件差0、
                run-examples 24本。
        - [x] **性質検査 第2弾: `mesh check` が黙ったならコード生成も成功する**
              ✅ 2026-07-28実装(`rust/tests/check_implies_build.rs`)。期待値を持たず
              **自己矛盾だけ**を見る性質。考え方は `docs/handoff.md`「性質検査(オラクルの
              代わり)」節が一次情報源。
              - **なぜこの性質か**: Meshは`mesh check --json`をエージェント向けの構造化診断
                として出している。**checkが「問題なし」と言ったのにビルドできない**のは
                その約束自体が破れている状態。しかも構造的に破れやすい——`mesh check`は
              ...
              - **残り2件は「直そうとして取り下げた」**。ここが今回いちばん学べたところ。
                ...
                埋まる——**実際に埋まったが、同時にもっと悪いものを作った**。
                `full_checker.rs`はTS版の`invalidatePath`(絞り込んだパスへ代入したら事実を
                捨てる)を移植し忘れており、codegenがその判定をそのまま真似た結果、
                `if o.inner.v is int { o.inner.v = none; print(o.inner.v + 1) }` が
                **checkもbuildも通って実行時に静かに`1`を出す**ようになった
                (`null + 1 === 1`が`__iarith`のsafe-integerガードをすり抜ける)。
                一般化前はcodegenが`invalid operation: int | none + int`で**安全に失敗**
                していた形で、**「うるさい失敗」を「静かに誤った出力」に変えるのは退行**。
                main と #109 の両方のバイナリで実測して確認したうえで一般化を取り下げた。
                **順番はinvalidatePathの移植が先**。プローブは
                `tests/parity/narrow-then-assign-field/`。
              - **教訓**: 2つのcheckerが食い違っているとき、**どちらが正しいかを決めずに
                片方をもう片方へ揃えると、揃えた先が間違っていた場合に事故になる**。
                この性質検査は「食い違い」は見つけるが「どちらが正しいか」は答えない。
                なお`invalidatePath`の移植も素直にやると`o.inner.v += 1`で新しい誤検知を
                生みうる(無効化の前に検査するか後にするかで、どちらかが誤検知になる)
                ——オラクルが無い今、ここは実測ではなく**設計判断**が要る領域。
              - **副産物: 別の誤検知を発見(未修正)**。直した機能の説明が正しいかを実測する
                過程で、`docs/features.md`が「F-6で`a is Foo && a.value > 0`は左の`is`が
                **右辺の式**に効く」と書いている形が**Rust版では効かない**ことが分かった
                ——`full_checker::infer_binary`が`&&`/`||`の右辺を絞り込み前のスコープで
                検査するため`narrow-required`という誤検知になる。**誤検知は最悪の分類**なので
                次の優先候補。原因は典型的なdriftで、`infer_binary`直前のコメントが
                「unionはANYに潰れるので絞り込みの有無で結果が変わらない」と説明しているが、
                unionをモデル化したmilestone 39以降その前提は偽になっていた。
                プローブは`tests/parity/logical-and-narrow-right-operand/`。
              - **穴を塞ぐ軸を`sweep_gen.py`へ追加**(`cond_*`)。既存の`CONDS`は
                `x is int && 1 == 1`のように**右辺が絞り込みと無関係**な形しか作らないため、
                この族は直積を回してもまるごと出てこなかった。軸を足したら
                **生成した6件がすべて誤検知**だった——subjectが裸の識別子でもフィールドパス
                でも、比較(`> 0`)でも算術(`+ 1 > 0`)でも、`&&`でも`||`(De Morgan)でも
                同じように壊れている。1つの形ではなく族だった。
                `tests/sweep-expected.txt`には誤検知のまま記録してある(あの記録は
                「今の出力」であって「正しい出力」ではない)。直せば記録から診断が消えるので、
                そのときの差分が修正できた証拠になる。sweepは120→126件。
              - 破れ4件は`KNOWN_VIOLATIONS`に**理由つきで**登録した。
                **リストは両方向に効く**——新しい破れが出れば落ちるし、載せたケースが
                通るようになっても落ちるので、直したのに消し忘れることはできない。
                両方向とも実際に落ちることを確認済み。
              - 判定は`--emit-js`(生成JSを標準出力へ)。**JSランタイムも出力ファイルも要らない**
                ので速く、CIの構成に依存しない。診断が出るケースは前提を満たさないので対象外
                (**53件が検査対象、105件が対象外**——examples 24本 + tests/parity 134件が母数)。
                **黙って絞らず件数を必ず出す**。
              - 検証: テスト679件、clippyクリーン、parity 158件差0、sweep 126件差0、
                run-examples 24本。**`KNOWN_VIOLATIONS`は5件**(codegen未対応2 +
                フィールドパス絞り込み2 + invalidatePath移植漏れ1)。
        - [x] **milestone 67: コーパスの網羅性を仕組みにする(構文カバレッジ+組み合わせスイープ)**
              ✅ 2026-07-27実装。milestone 65で「コーパスに無い形は測れていない」と分かった点への対応。
              **調査で終わらせず、2つの恒久的な仕組みにした**。
              - `rust/tests/corpus_coverage.rs`: **ASTのどの変種がコーパスに一度も現れないか**を
                検出する。判定は`format!("{:?}", program)`に変種名が出るかで行う(専用visitorより
                遥かに短く、変種を足したとき自動で追随する)。`ast.rs`の変種一覧とズレたら
                落ちる別テストも置いた——ズレるとカバレッジを過小評価する。
                実行したところ**未使用は`Continue`の1件だけ**で、コーパスは変種単位では
                ほぼ網羅できていた。
              - **変種単位では足りない**——milestone 65の穴(`!`/`&&`/`||`で包んだ条件式)は
                単体では全部コーパスにある要素の**組み合わせ**だった。そこで
                `mise run sweep`(`scripts/sweep.sh` + `sweep_gen.py`)で軸の直積を機械生成して
                突き合わせる形にした。4種類の直積・**120件で誤検知0・検出漏れ0**。
                - 値の作り方(型注釈/関数戻り値/map読み/chan受信)× 条件の書き方6種 × 使い方2種
                - 値を作る式6種 × 型の食い違いを誘発する使い方4種
                - 文の種類8種 × 中で起こすエラー4種
                - match/select/orelseのアーム4種 × 4種
              - **`sweep.sh`はパースできない生成物(SKIP)があると失敗する**。milestone 62で
                19件中3件が空振りしていたのに一致として数えていた事故への対処。
              - 検証: `mise run parity` 156ファイル・検出漏れ0・誤検知0・生成JS差0。
                テスト629件(カバレッジテスト2件を含む)、clippyクリーン。
