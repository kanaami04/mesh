        ...
        - [x] **`invalidatePath` の移植(絞り込んだパスへ代入したら事実を捨てる)**
              ...
              **診断ゼロ**になっていた(検出漏れ)。`docs/features.md` が「フィールドへの代入は
              ...
              - **オラクルは消えたがTS版のソースは git 履歴に残っている**、というのが今回の
                ...
                判断そのものが不要になった**——手順は `docs/handoff.md`「TS版のソースは
                git 履歴に残っている」節。
              ...
              - **記録の差分は1件だけ**——`tests/parity/narrow-then-assign-field/` が
                診断ゼロから `invalid-operation` になった。158件中の他は無変更で、
                狙った検出漏れだけが埋まったことの証拠になっている。複合代入
                (`i.v += 1`)も誤検知にならないことを実測で確認。
              - **副産物: `KNOWN_VIOLATIONS` の腐り検出の穴を塞いだ**。登録ケースが
              ...
              - 検証: テスト679件、clippyクリーン、parity 158件差1(上記の意図した差分のみ)、
                sweep 126件差0、run-examples 24本。
