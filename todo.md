        ...
        - [x] **性質検査 第2弾: `mesh check` が黙ったならコード生成も成功する**
              ...
              **自己矛盾だけ**を見る性質。考え方は `docs/handoff.md`「性質検査(オラクルの
              ...
              - **導入時点で4件破れていた**。2件はcodegen側の明示的な「未対応」
                ...
                **残り2件はこの検査で初めて見つかった移植漏れで、同じ回で修正した**——
                `checker.rs`(codegen側の最小リゾルバ)がフィールドパスの絞り込みを持たず、
                `full_checker`が通したプログラムをcodegenだけが弾いていた
                (言語機能としてはF-6で実装済み)。絞り込みのキーを識別子名から
                `checker::stable_path`(`a` / `a.b` / `a.b.c`)へ一般化して`full_checker`と揃えた。
                識別子のときの挙動は変わらない(`stable_path`は識別子に名前そのものを返す)。
                **2件目は気づきにくい形だった**——`b.next`が絞り込まれず`Node | none`のまま
                なので、フィールド読みのガード(structだと分からない=パッケージ参照だろう)に
                引っかかり`package/member access is not yet supported`という**根本原因と
                無関係なメッセージ**で落ちていた。回帰テスト2件を`codegen.rs`に追加。
              ...
              - 4件は`KNOWN_VIOLATIONS`に**理由つきで**登録した。**リストは両方向に効く**
                ——新しい破れが出れば落ちるし、載せたケースが通るようになっても落ちるので、
                直したのに消し忘れることはできない。両方向とも実際に落ちることを確認済み。
              - 判定は`--emit-js`(生成JSを標準出力へ)。**JSランタイムも出力ファイルも要らない**
                ので速く、CIの構成に依存しない。診断が出るケースは前提を満たさないので対象外
                (28件が検査対象、104件が対象外)。**黙って絞らず件数を必ず出す**。
              - 検証: テスト679件、clippyクリーン、parity 156件差0、sweep 120件差0、
                run-examples 24本、drift 候補0。
