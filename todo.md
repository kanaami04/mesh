...
- [ ] **批評ターン(2026-07-19)起点の再討議・実装** — 全記録は [docs/critique-2026-07.md](docs/critique-2026-07.md)、
  ...
  - 推奨順: ~~F-1前半 関数型注釈~~・~~F-2前半 `? "文脈"`~~・~~F-4 `?`改名~~・~~F-5 or束縛形~~
    (すべて✅ 2026-07-19)→ F-3 構造的型付けの範囲 → F-6 narrowing拡張
    → F-1後半 ジェネリクス・F-2後半 構造化エラー → F-12 対TS/Go比較ベンチ
  - 応急処置(討議不要・即日可能): narrowingが効かないパターンのカード明記(F-6の前半)、
    カード完全性の逆方向検査(`s[0]`未記載問題、F-13の一部)
