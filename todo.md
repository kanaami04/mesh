  ...
  - [ ] **フルchecker(診断)+CLI移植** — 2026-07-24提起・方針合意。**milestone 22(第一歩)は
        ...
        - [x] **性質検査 第2弾: `mesh check` が黙ったならコード生成も成功する**
              ...
              **自己矛盾だけ**を見る性質。考え方は `docs/handoff.md`「性質検査(オラクルの
              ...
              - **導入時点で4件破れていた**。3件はcodegen側の明示的な「未対応」
                (パッケージ修飾レシーバ / パッケージ修飾の値参照)で既知。
                **残り1件はこの検査で初めて見つかった移植漏れ**——入れ子のフィールドパス
                (`if o.inner.v is int { o.inner.v + 1 }`)の絞り込みが`checker.rs`に無く、
                `invalid operation: int | none + int`になる。言語機能としてはF-6で実装済みで、
                `full_checker.rs`は正しく絞り込む。**次に片付ける候補**(`docs/features.md`の
                narrowingの行にも既知の穴として明記した)。
