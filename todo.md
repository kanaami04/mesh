...
- [ ] **言語カード実証実験の継続** — 全記録は [docs/card-experiments.md](docs/card-experiments.md)
  ...
  - [ ] **`mut best := none` 問題(設計判断)** — 不在から始まる可変アキュムレータが書けない。
        `none` 単体宣言は型が `none` に固定され後続代入を拒否。absence が売りなのに命令型で
        「不在から積み上げる」ができない。対策候補: (a) 型注釈つきローカル宣言
        `best: string | none = none` を導入 / (b) `mut`+`none` を特別扱い。要決定
  - [ ] 第4回は上記対策後に再実験、または別題材(簡易パーサ等)で新しい穴探し
...
- [ ] **標準ライブラリ** — 言語カード実験(2026-07-18)で必要性が浮上。
      現状ビルトインは print/len/push/str/error/sleep/delete のみで、配列の find/filter/map、
      文字列操作(split/join/trim)、数値↔文字列変換(parseInt等)、ソートが全て手書きになる。
