...
- [ ] **言語カード実証実験の継続** — 2026-07-18に第1回実施(白紙AI+カードでTODOアプリ→6往復で完成)
  - [x] 空の型付き配列 `Todo[]{}`(実験で最大の詰まり=実装の穴だった)
  - [x] カード追記: 空配列・push はnoneを返す・struct参照セマンティクス・errメッセージ補間
  - [ ] 同じ実験を再実行し往復数が減るか確認(6往復→?)。別題材(集計/パーサ等)でも試す
