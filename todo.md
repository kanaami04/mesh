...
- [ ] **批評ターン(2026-07-19)起点の再討議・実装** — 全記録は [docs/critique-2026-07.md](docs/critique-2026-07.md)、
  ...
  - [x] ~~F-13前半(診断コード+`fix`パッチ+`mesh explain`)~~ ✅ 2026-07-20実装。
        全169箇所の診断に約87種のコードを付与、機械適用可能なものに単一range置換の`fix`、
        `mesh explain <code>`で説明を引ける
  - **F-13後半(未着手)**: サブセットカード(プロジェクトで使っている機能だけの抜粋カード生成)、
    カード完全性の逆方向検査(実装がカードからはみ出したらCIで落とす。`s[0]`未記載が過去の実例)
