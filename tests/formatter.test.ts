...
describe("mesh fmt: べき等性・意味保存(examples全体)", () => {
  ...
  for (const f of files) {
    test(`${f}: 整形後も同じ出力・整形は収束する`, () => {
      ...
      // examples/jsonmodels を、defer_pkg_demo.mesh は examples/loggerpkg を使う)
      ...
      if (f === "defer_pkg_demo.mesh") {
        cpSync(join(examplesDir, "loggerpkg"), join(dir, "loggerpkg"), { recursive: true });
      }
...
describe("退行防止: mesh fmtが意味を変えてしまっていたバグ(見つかって直したもの)", () => {
  ...
  test("'defer'文は再整形しても消えない(milestone 11のexample追加で発覚)", () => {
    // printStmtのswitchにdeferStmtのcaseが無く(default節も無いため型検査でも
    // 検出できなかった)、defer文を再整形すると文そのものが丸ごと消えてしまっていた
    // (examples/defer.meshのフォーマッタ往復テストで発覚)
    const src = `fn main() {\n\tdefer print("cleanup")\n\tprint("body")\n}\n`;
    const out = format(src);
    expect(out).toContain('defer print("cleanup")');
