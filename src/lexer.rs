...
pub enum ErrorCode {
    ...
    E0102,
    /// 識別子位置の非ASCII字母(仕様1章L-6)。spanは連続する非ASCII字母の全体。
    /// 字母でない非ASCII(記号・Unicode改行類)はE0116のまま。
    E0103,
    ...
    /// 注意: 識別子を構成し得ない非ASCII文字(記号 `±`・Unicode改行類)は
    /// 字母でないためこのコードのまま(非ASCII**字母**はL-6のE0103が担う)。
...
fn next_token(
        ...
        && peek_at(source, start + '.'.len_utf8()).is_some_and(|d| d.is_ascii_digit())
    {
        ...
        // 識別子位置の非ASCII**字母**(仕様1章L-6)。ASCII識別子を期待する位置に現れた
        // 非ASCII字母は、英数字名への変更を促すE0103として報告する。
        // spanは連続する非ASCII字母の全体(`let 合計 = 0` の `合計` 6バイト。
        // 先頭1文字だけだと修正範囲が伝わらないため、scan_whileで一気に測る)。
        // 字母に限るのは、識別子を構成し得ない非ASCII(記号 `±`・Unicode改行類U+2028等)を
        // E0116(L-26・L-29)のまま保つため。文字列リテラルとコメントの内側の非ASCIIは
        // それぞれの分岐が読み飛ばすため、ここに到達する非ASCII字母は識別子位置のみ。
        let (_, end) = scan_while(source, chars, start, c, |d| {
            !d.is_ascii() && d.is_alphabetic()
        });
        Err(LexError {
