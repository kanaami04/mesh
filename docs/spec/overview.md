...
- **3章からの申し送り**: ~~4章=continue/breakのnarrowing+ヘッダstructリテラル~~(**4章S-17/S-15で消化済み**)。~~5章=関数本体の値位置~~(**5章F-5で消化済み。暗黙return採用=ADR-0036**)。**7章への申し送り**: 未定義の名前(変数・関数)の解決規則とエラー(5章F-1のローカル再帰不可の根拠を含む)、mainの配置。
