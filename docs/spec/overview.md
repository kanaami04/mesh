...
| 章 | 内容(1〜7章は執筆済み=確定。8章以降のカッコ内はたたき台) | 状態 |
|---|---|---|
...
## 構文スケッチ(コア構文は1〜7章で**確定済み**。`component`/`state`/`view` のUI部のみ9章のたたき台)
...
- **11章への申し送り**: mapの構成手段(リテラルまたは組み込み。7章M-10の帰結でトップレベルmap定数が現状構成不能)、**スライスの定義**(ADR-0014とS-9のエラーメッセージ文言が参照しているが未定義)。

## 仕様の書き方ルール

- 各章は「文法(EBNF風)+ 意味 + 正例 + **負例(コンパイルエラーになるべき例)**」の4点セットで書く。
- 意味の記述は**EARS風の受け入れ基準**で書く(例:「matchの腕がunionの全ケースを網羅していないとき、コンパイラはエラーE0315を報告すること」=3章X-16)。曖昧な散文でなく、そのままテストに落ちる文にする。
