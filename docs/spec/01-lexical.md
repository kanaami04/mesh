...
- **L-6**: 識別子はASCIIのみ。ASCII外の**字母**(Unicodeの Alphabetic 特性を持つ文字)が識別子位置に現れたとき、エラー E0103 を報告し、英数字名への変更を促すこと。**spanは連続する非ASCII字母の全体を指すこと**(`let 合計 = 0` なら `合計` の6バイト。先頭1文字だけを指すと修正箇所が伝わらない)。字母でない非ASCII文字(記号 `±`・Unicode改行類・全角空白・全角数字)はL-26のE0116が担う——「英数字名への変更を促す」案内がそれらには意味をなさないため(ADR-0041決定3・ADR-0048)。文字列リテラルとコメントの日本語は制限しない。〔負例: `non-ascii-ident`(`let 合計 = 0`)〕
