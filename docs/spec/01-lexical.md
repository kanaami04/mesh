...
- **L-18**: `${` が対応する `}` を得ないままリテラル・行が終わる、または補間内の括弧が**不均衡**なまま終わるとき、エラー E0109 を報告すること。「不均衡」とは、対応の無い閉じ括弧(対応待ちの開きが無い `)` `]`)または**種類の合わない**閉じ括弧(`(` に対する `}` 等。対応は括弧の種類も照合する)が現れた状態を指し、このときのspanはその閉じ括弧を指す。**補間の内側ではE0109がL-16(E0108)より優先する**。〔負例: `unterminated-interpolation`〕
  - 注1: 補間の内側で発生した**他の字句エラー**(ネスト文字列のE0108/E0111/E0112等)は、E0109より優先して報告する(より具体的な原因を指すほうが修正しやすい。実装は内側エラーの素直な伝播。E0115とは出現順で先のものが報告される)。
  - 注2: 例外として、補間の内側の**単独CR**はE0117でなくE0109に統一する(LF・CRLF・単独CRの3形が同じ「物理1行制約の違反」として報告される。ADR-0041のエラー経路との相互作用)。
