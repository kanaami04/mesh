...
- **L-1**: ソースファイルがUTF-8として不正なとき、コンパイラは位置つきでエラー E0101 を報告すること。**この検査は読み込み層が行う**——字句解析器は妥当なUTF-8を受け取る前提で組まれており、不正バイトはファイルを読んだ時点で判明するため(位置は妥当なUTF-8として読めた末尾を行・列に換算する。算出手段はADR-0048)。〔負例: `invalid-utf8`〕
- 拡張子は `.mesh`。
...
- **L-6**: 識別子はASCIIのみ。ASCII外の**字母**(Unicodeの Alphabetic 特性を持つ文字。判定はコンパイラが用いるUnicodeテーブルに従う)が識別子位置に現れたとき、エラー E0103 を報告し、英数字名への変更を促すこと。**spanは連続する非ASCII字母の全体を指すこと**(`let 合計 = 0` なら `合計` の6バイト。先頭1文字だけを指すと修正箇所が伝わらない)。字母でない非ASCII文字(記号 `±`・Unicode改行類・全角空白・全角数字)はL-26のE0116が担う——「英数字名への変更を促す」案内がそれらには意味をなさないため(ADR-0041決定3・ADR-0048)。文字列リテラルとコメントの日本語は制限しない。〔負例: `non-ascii-ident`(`let 合計 = 0`)〕
...
- **L-26**(キャッチオール): この一覧・本章のどの規則にも該当しない文字・記号列はエラー E0116 とし、近い正解があれば案内すること: `===` →「等価比較は `==`」/ `!==` →「非等価比較は `!=`」/ `->` →「戻り値型は空白のみ・matchの腕は `=>`」/ `` ` `` →「文字列は `"`(`"` でも `${}` 補間が使えます)」/ `'` →「文字列は `"`」。`;` はL-19が、単独CRはL-29が、**非ASCII字母はL-6が**優先する。〔負例: `triple-equals`、`triple-not-equals`、`arrow-token`、`backtick-string`、`single-quote-string`〕
