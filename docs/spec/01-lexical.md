...
- **L-12**: 次の形はエラー E0113 とし、それぞれ修正候補を提示すること: 指数部が空(`1e`)/ 基数接頭辞の後に数字が無い(`0x`)/ 基数外の数字(`0b102`。最長一致で `0b10`+`2` に割らず、リテラル全体の不正として診断する)/ 先頭ゼロの10進(`0755`・`0755.5`・`01e5`。C系8進の誤解防止として `0o755` か `755` へ誘導)/ 数字の直後に識別子文字(`123abc`)。〔負例: `number-malformed`〕
