...
- **L-10**: floatの小数点の**両側には数字が必須**であること。`0.` と `.5` はエラー E0106 とし、`0.0` / `0.5` への修正候補を提示すること(設計監査#12)。〔負例: `float-dot-edge`〕
  - 注(`.` の帰属): 数字の直後の `.` は、直後がもう1つの `.`(L-3の `..`)である場合を除き**常に数値リテラルの小数点**として扱う。したがって `1.abs` もE0106である(メンバアクセスの `.` とは解釈しない)。数値リテラルへのフィールドアクセスは3章X-30がstruct型に限るため、この解釈で正当なプログラムは失われない。spanは「数字列+`.`」(`0.`)または「`.`+数字列」(`.5`)の全体を指す。
