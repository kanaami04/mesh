...
- **S-23**(発散の定義): 文が**発散する**とは、次のいずれかであること: (a) `return`・`continue`・`break`、(b) `else` を持ち両腕の最終文が発散するif(elseの腕がifFormのときはその腕自身に本定義を再帰適用する=else-if連鎖)、(c) 全腕のブロックの最終文が発散するmatch、(d) 本体に(その`for`自身を抜ける)`break` を含まない無限ループ `for { }`。S-1・X-13・X-25・F-6(5章)の「発散」はこの定義を参照する。〔正例テスト: 5章 `return-everywhere`〕
- **S-1**: ブロックが値位置(X-12)にあるとき、ブロックの値は**最終文の式**の値であること。最終文が**発散する**(S-23)場合、そのブロックは値の要求から除外される(if/matchの他の腕が型を決める。X-13)。最終文が式でも発散もしない(let・代入で終わる)場合、および空ブロックの場合はエラー E0408 とし、値を返す式で終えるよう案内すること(関数本体ではF-6のE0504が優先=5章)。〔負例: `block-no-tail-expr`(let終わり・空ブロックの2形)〕
...
- **S-4**(シャドーイング禁止。ADR-0008): 可視な既存の値の名前(外側のブロックのlet/mut・パラメータ・トップレベル宣言・ループ変数・**importのパッケージ参照名**=7章M-15)と同名の宣言はエラー E0402 とし、リネームを提案すること。ネストしたブロックでも禁止。matchパターン束縛の同違反はE0317(3章X-18。章別コード空間のため番号が異なるが同一規則)。**値と型は別の名前空間**であり、`struct User` と `let user` は共存できる。〔負例: `shadowing`(ネストブロック・パラメータの2形)〕
...
- **S-6**(未使用変数): 宣言した変数(ループ変数を含む)が一度も**読まれない**とき、エラー E0403「未使用です。不要なら削除、意図的な破棄なら `_`」を報告すること(ADR-0035)。書き込み(再代入)のみで読まれない変数も未使用である(Goと同じ)。**自己への複合代入(`count += 1`)に含まれる読みは「使用」に数えない**(値が他の場所で読まれて初めて使用。書き込み専用カウンタの放置を検出するため)。**関数のパラメータ・レシーバは対象外**(未使用でもエラーにしない。5章F-3)。**トップレベルletも対象外**(公開定数のため。7章M-12)。〔負例: `unused-variable`(再代入のみの変数を含む)〕
...
- **S-17**(narrowingの引き継ぎ。3章X-21の適用範囲): X-21の「片側の腕の最終文が発散する(S-23)とき」の規則について、発散が `continue`/`break` の場合の適用範囲を定める: continueなら同一反復内のif文以降、breakならループ後は対象外(ループを抜けた後の型はループ前に戻る)。
...
- **S-18**: `return [expr]` は関数本体の中でのみ書けること(トップレベルはエラー E0410)。値なし `return` は戻り値型が `none` の関数でのみ書け、`return none` と同じ意味(**制限つき短縮形**。可否の規則は5章F-7)。**無名関数の中のreturnはその無名関数からのreturn**(外側の関数ではない。型検査・関数本体末尾の扱いとあわせて5章が定める)。〔負例: `return-top-level`〕
