...
「いまは決めない・やらないと**意図的に**判断した事柄」の一覧。トリガー欄が「いつ読むか」を持ち、`file:` の行は PreToolUse フックが着手の瞬間に提示する(ADR-0049)。前提台帳が「崩れるかもしれない前提」を追うのに対し、こちらは「いつか判断する先送り」を追う。
...
- [ADR-0048](0048-utf8-check-belongs-to-reader.md) — 不正UTF-8は読み込み層の責務・E0103は非ASCII字母限定・誤綴り誘導はPhase 9判断 ✅
- [ADR-0049](0049-deferred-ledger-and-bracket-mapping.md) — 先送りは申し送り台帳(DEFERRED.md)に集めトリガーで撃つ・括弧の写像はBracketKindに統一 ✅
