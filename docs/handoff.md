...
**「実行できるオラクル」は失ったが「読める仕様」は残っている**。移植漏れを埋めるとき、
挙動を推測したり設計し直したりする前に、まずこれを読むこと。実際、`invalidatePath`では
「複合代入で誤検知になるのでは」という懸念を抱えて着手したが、TS版のコメントに
**なぜ無効化を型検査の前に置くのか**(順序が逆だと宣言型には代入できるはずの値が弾かれる)
が書いてあり、設計判断そのものが不要になった。
