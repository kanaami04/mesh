...
- todo.md・features.md の未着手項目から選ぶ: 標準ライブラリ層分け(C-6)、モジュールシステム
  (import/export)、defer文、エラー表示改善(ソース行表示・複数エラー報告)、mesh fmt、Rust移植
- ※ `is` の対象拡大は 2026-07-19 実装済み(matchと完全に同じパターン)
