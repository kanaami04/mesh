...
- `rust/tests/check_implies_build.rs`「**`mesh check` が診断ゼロならコード生成も成功する**」。
  ...
  **checkの段階で止まる**。codegen側のフィールドパス絞り込み(`KNOWN_VIOLATIONS`の2件)は
  これで安全に再挑戦できる状態になった。
...
**次に片付けたいもの(2026-07-28発覚、未修正)**: **`&&`/`||`の右辺に絞り込みが効かない誤検知**。

```mesh
if a is Foo && a.value > 0 {   // → error[narrow-required] が出る(誤検知)
```

`docs/features.md`が「F-6で左の`is`が**右辺の式**とthen節の両方に効く」と書いている形が、
Rust版では効かない。`full_checker::infer_binary`が`&&`/`||`の右辺を絞り込み前のスコープで
検査するため。**誤検知はこのプロジェクトで最悪の分類**なので優先度は高い。

見つけ方が示唆的で、性質検査そのものではなく**「直した機能の説明が正しいか」を実測した副産物**
だった。そして原因は典型的なdriftで、`infer_binary`直前のコメントが
「unionは`resolve_scalar_type`でANYに潰れる——ANYは全チェックの安全弁を素通りするので、
絞り込みの有無で結果が変わらない」と説明しているが、**unionをモデル化したmilestone 39以降
この前提は偽**になっていた。前提が消えたのに結論だけが残った形。

修正方針: codegen側の`checker.rs`には`check_logical_op`のスクラッチctxという同じ問題への
対処が既にある(裸の識別子のみ対応)。同じ技法を`full_checker`へ持ち込む。
プローブは`tests/parity/logical-and-narrow-right-operand/`に置いてあるので、
直せば`mise run parity`の差分に誤検知が消えるところが出る。

**穴の広さ**: `sweep_gen.py`に「**絞り込んだ値を条件式の右辺で使う**」軸を足したところ
(`cond_*`、2026-07-28)、**生成した6件がすべて誤検知だった**。1つの形ではなく、
subjectが裸の識別子でもフィールドパスでも、比較(`> 0`)でも算術(`+ 1 > 0`)でも、
`&&`でも`||`(De Morgan)でも同じように壊れている。既存の`CONDS`は
`x is int && 1 == 1`のように**右辺が絞り込みと無関係**な形しか作らないため、
直積を回してもこの族はまるごと出てこなかった。

**`tests/sweep-expected.txt`にはこの6件が誤検知のまま記録されている**——あの記録は
「今の出力」を凍結するもので「正しい出力」ではない。直せば記録から診断が消えるので、
そのときの差分が修正できた証拠になる。
