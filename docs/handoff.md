...
- `rust/tests/check_implies_build.rs`「**`mesh check` が診断ゼロならコード生成も成功する**」。
  ...
  **導入時点で4件破れていた**(`KNOWN_VIOLATIONS`に理由つきで登録。リストは両方向に効くので、
  直したのに消し忘れることはできない)。**うち2件は同じ回で直した**——`checker.rs`が
  フィールドパスの絞り込みを持たず、`full_checker`が通したプログラムをcodegenだけが弾いていた
  (言語機能としてはF-6で実装済み)。絞り込みのキーを識別子名から`stable_path`へ一般化して解消。
  2件目は`package/member access is not yet supported`という**根本原因と無関係なメッセージ**で
  落ちており、同じ穴だと気づける形になっていなかった。残る2件はcodegen側の本物の「未対応」
...
**次に片付けたいもの(2026-07-28に実測で発覚、未修正)**: **`&&`/`||`の右辺に絞り込みが効かない誤検知**。
