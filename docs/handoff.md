...
- **テスト265件**(2026-07-19時点。F-6のnarrowing拡張テスト込み。`bun test`で最新件数を確認)、CI green
...
- **批評起点のF節**(design-agenda参照)が最優先。推奨順:
  ~~F-1前半 関数型注釈~~・~~F-2前半 `? "文脈"`~~・~~F-3 名前的へ巻き戻し~~・~~F-4 `?`改名~~・
  ~~F-5 or束縛形~~・~~F-6 narrowing拡張(フィールドパス・`&&`・`!`/`||`)~~(すべて✅ 2026-07-19)
  → F-1後半 ジェネリクス・F-2後半 構造化エラー → F-12 対TS/Go比較ベンチ
- **C-6の続き**: 環境別標準ライブラリ(`mesh/http`・`mesh/dom`等)の中身の設計(=F-14)。
  Q3(フロントエンドの形)の討議とセット。F-1が先行依存
