...
## 次のセッションでやること(2026-07-29時点)

**直近(milestone 68)で`rust/tests/check_implies_build.rs`の`KNOWN_VIOLATIONS`が0件になった**
——「checkが黙ったのにbuildが落ちる」既知の破れは残っていない(下記「既知の破れ〜を0件にした」節)。
そこで見つかった穴が示しているのは、**記録(parity/sweep)には原理的に届かない領域がある**
ということ:代入文の診断は**コーパスに1件も無く**、TS版と4通り食い違ったまま何回もCIを
通り続けていた。**移植漏れを疑うときは記録ではなくオラクルに聞く**(下記の復元手順)。

**次の一手の候補**(おすすめ順):

1. **性質検査 第3弾**。`docs/handoff.md`の「次の候補」にある4つのうち、**オラクル非依存**の
   ものから着手する(整形・変数の一貫リネームで**実行結果**が変わらない / どんな壊れた入力でも
   panicしない)。「ビルドできたプログラムは実行して期待どおりの結果を出す」は、そのままだと
   「期待どおり」の判定に答えが要るのでオラクル非依存にならない——**メタモルフィックな形に
   言い換えてから**着手すること
2. **最初の新機能に着手する**(= オラクルを捨てる決断とセット。下記「loop engineeringの
   見取り図が変わる時期」)
3. **言語カード実証実験 第12回**(大規模複合タスク)

**2026-07-27にTS実装(`src/`)を撤去した。** 撤去条件(1)〜(5)をすべて満たしたうえで、
段階1〜5を順に実施している(詳細は `docs/ts-removal-plan.md`)。
...
**`--update`/`UPDATE_SNAPSHOTS=1`で更新したら、差分を必ず読むこと**——
説明できない変化は退行の可能性が高い。

**記録の弱点を補う「性質検査」を始めた(2026-07-28)。** 記録(スナップショット)は
オラクルと違って**新しい入力に答えを出せず**、`--update`で自分の出力をそのまま
正解にできてしまう。そこで「**答えを知らなくても正誤を判定できる**」検査を足していく:
意味を変えないはずの変換を通して、診断が変わらないことを確かめる(メタモルフィック検査)。
初弾は`rust/tests/fmt_corpus.rs`の「整形は診断を変えない」(コーパス156本)。
**実在の不具合を1件見つけた**——`mesh fmt`が`!(x is none)`を`!x is none`に整形して
意味を変えていた。詳細と設計の考え方は下記「性質検査(オラクルの代わり)」節。
...
メンバー参照とメソッド呼び出しで別々にフォールバックしていた)。**milestone 48で型宣言の
名前衝突**(`already-declared` / `builtin-type-redeclared` / `name-conflicts-with-package`)
**も移植済み**——検証スイープで一番よく当たっていた検出漏れが片付いた。その調査で**未移植だと
確定した診断**が下記候補に反映してある。
...
**コーパスの網羅性はmilestone 67で2つの仕組みにした**(それまでは「意識して作る」しか無く、
milestone 65の誤検知5種を長く見逃していた):

1. `rust/tests/corpus_coverage.rs` — **ASTのどの変種がコーパスに一度も現れないか**を検出する。
   `ast.rs`へ変種を足したら「一覧がずれている」で落ちるので、追随を忘れられない
2. `mise run sweep`(`scripts/sweep.sh` + `sweep_gen.py`)— **軸の直積を機械生成して
   TS版と突き合わせる**。milestone 65の穴は「単体では全部コーパスにある要素の組み合わせ」
   だったので、変種カバレッジでは検出できない。現在120件・差0件

**直積そのものはコーパスへ入れない**(数千件になり回らなくなる)。入れるのは
**差が出た形と代表例だけ**で、直積は「検査の効き方を変えたときに手で一度回す」使い方。
`sweep.sh`は**パースできない生成物(SKIP)があると失敗する**——milestone 62で19件中3件が
空振りしていたのに一致として数えていたため。
...
2-0. **`mise run parity` と `mise run drift` を回す**(2026-07-27にスクリプト化)。
   下記2'・2''を毎回手で組み立てていて取りこぼしたので、判定と検索語の作り方を固定した。
   - `mise run parity` — `examples/` と `tests/parity/`(48ケース)を両CLIに通し、
     **Rust側だけに出る診断があれば失敗**。検出漏れは情報として出すだけ。
     **プローブはリポジトリに置く**(`tests/parity/<ケース>/main.mesh`。サブディレクトリを
     置けばパッケージ跨ぎも書ける)——`/tmp`に作ると毎セッション消えて、実際に似たものを
     6回作り直した。新しい診断を移植したらケースを足して `mise run parity-update`
   - **CIはRust側だけ**(`rust/tests/parity.rs` が `expected.txt` と突き合わせる)。
     bun不要で速く、**診断の変化がPRのdiffに出る**のが利点。expected.txtを手で書き換えない
     ——オラクルとの照合を飛ばすことになる
   - `mise run drift` — 変更から検索語を作って候補を出す。**判定はしない**
...
### 性質検査(オラクルの代わり。2026-07-28開始)

TS版を撤去して失ったのは「テストの本数」ではなく**オラクル**——*新しい入力にも答えを
出せる相手*だった。残った記録(スナップショット)には2つの弱点がある:

| | オラクル(TS版) | 記録(スナップショット) |
|---|---|---|
| **新しい入力**に答えを出せる | できる | **できない** |
| 更新に人間/Claudeの判断が要る | 要らない | **要る**(`--update`の差分を読む) |

とくに2つ目が効く。`--update`は**自分の出力をそのまま正解として焼き込める**ので、
記録だけを信号にして自走すると退行が静かに正解になる。

そこで**答えを知らなくても正誤を判定できる**検査を足していく。「意味を変えないはずの
変換」を通し、変換の前後で診断が一致するかを見る(メタモルフィック検査)。期待値ファイルを
持たないので、生成する形をいくら増やしても`--update`の抜け道が構造的に存在しない。

**いま入っているもの**:

- `rust/tests/check_implies_build.rs`「**`mesh check` が診断ゼロならコード生成も成功する**」。
  期待値を持たず、**自己矛盾だけ**を見る。Meshは`mesh check --json`をエージェント向けの
  構造化診断として出しているので、**checkが「問題なし」と言ったのにビルドできない**のは
  その約束自体が破れている状態。構造的な理由があって破れやすい——`mesh check`は
  `full_checker.rs`、コード生成は`checker.rs`(最小リゾルバ)という**別々の実装**を通るため、
  片方だけが知っている機能があると食い違う。
  **導入時点で4件破れていた**(`KNOWN_VIOLATIONS`に理由つきで登録。リストは両方向に効くので、
  直したのに消し忘れることはできない)。2件はcodegen側の本物の「未対応」
  (パッケージ修飾レシーバ / パッケージ修飾の値参照)。
  **2026-07-29(milestone 68)に`KNOWN_VIOLATIONS`は0件になった**(下記「既知の破れを0件にした」節)。

  **残り2件の顛末が、この検査で一番学べたところ**。原因は`checker.rs`(codegen側の最小リゾルバ)が
  裸の識別子しか絞り込まないことなので、キーを`stable_path`へ一般化すれば埋まる——**実際に
  埋まったが、同時にもっと悪いものを作ったので取り下げた**。`full_checker.rs`はTS版の
  `invalidatePath`(絞り込んだパスへ代入したら事実を捨てる)を移植し忘れており、codegenが
  その判定をそのまま真似た結果:

  ```mesh
  if o.inner.v is int { o.inner.v = none; print(o.inner.v + 1) }
  ```

  が**checkもbuildも通って実行時に静かに`1`を出す**ようになった(`null + 1 === 1`が
  `__iarith`のsafe-integerガードをすり抜ける)。一般化前はcodegenが
  `invalid operation: int | none + int`で**安全に失敗**していた形で、
  **「うるさい失敗」を「静かに誤った出力」に変えるのは退行**。

  **教訓**: 2つのcheckerが食い違っているとき、**どちらが正しいかを決めずに片方をもう片方へ
  揃えると、揃えた先が間違っていた場合に事故になる**。この性質検査は「食い違い」は
  見つけるが「どちらが正しいか」は答えない——そこは人間(とClaude)が決める。

  **前提条件だった`invalidatePath`は2026-07-28に移植済み**(下記「TS版のソースは git 履歴に
  残っている」節)。`mesh check`が`invalid operation`を出すようになったので、あのミスコンパイルは
  **checkの段階で止まる**。**同日中にcodegen側の絞り込みも入れ直し、今度は無効化を同時に
  入れて完了した**(下記「codegen側も片付いた」)。順番を守れば安全に入る、という実例になった。

  **`KNOWN_VIOLATIONS`の腐り方にも穴があった**(同日修正): 登録したケースが**診断を出すように
  なると性質の前提(診断ゼロ)から外れて黙って対象外へ落ち**、リストに残ったまま誰も気づけない。
  `invalidatePath`の移植でまさにそれが起きたので、リスト整合性テストに
  「登録ケースは`check`が黙っていること」を足した。
- `rust/tests/fmt_corpus.rs`「コーパス全体で整形は診断を変えない」(156本)。
  同ファイルの先行テストは意味の保存を**実行して標準出力を比べる**方法で見ており、
  **走らないプログラム(診断を出させるための`tests/parity/`132ケース)には原理的に
  効かなかった**。不具合はちょうどそこに住んでいた。しかも**べき等性は保たれたまま
  意味だけ壊れる**形だったので、べき等性の検査も素通りしていた
  ——**同じ「性質テスト」でも判定の置き方で見えるものが 24本 → 156本 変わる**

**書くときの注意(試作で実際に踏んだ)**:

- **等価性の主張は書き手の責任**。`sweep_gen.py`の`CONDS`には`!(x is none && 1 == 2)`
  という形があるが、これは常に真で絞り込み情報を持たないので`x is int`と**等価ではない**。
  等価だと思って入れると偽の失敗になる
- **性質検査が出すのは候補であって判定ではない**(`drift.sh`と同じ)。ただし決定的な違いが
  あり、**候補は一度判断すれば固定される**(等価集合から外すか、既知の非等価として記録する)。
  driftは毎回diffから作り直すので毎回読み直しになる
- 破れたときは**まず自分の等価性の主張を疑う**。試作では`x is int || 1 == 2`が
  絞り込まない件を不具合かと思ったが、`A || B`のthen節で成り立つ事実はAとBの積集合
  なので、定数畳み込みをしない限り絞り込めない(仕様どおりの可能性が高い)

### オラクル差分ハンター(`mise run oracle-hunt`、2026-07-28)

**段階2(自走ループ)の中身**。ランダムに生成した`.mesh`を Rust版とTS版オラクルの両方へ通し、
診断が食い違う形を探す。`scripts/oracle-hunt.sh` + `scripts/oracle_hunt_gen.py`。

|  | `mise run sweep` | `mise run oracle-hunt` |
|---|---|---|
| 生成 | 軸の**直積**(毎回同じ126件) | 軸を**ランダムに組み合わせ**(毎回違う形) |
| 判定 | **記録**(`sweep-expected.txt`)との一致 | **オラクル**との一致 |
| 抜け道 | `--update`で自分の出力を正解にできる | **無い**(判定はオラクルがする) |
| 用途 | CI・退行検出 | 探索 |

**シードを必ず出力する**のが生命線——差が出た形を再現できなければ調査に入れない。
`scripts/oracle-hunt.sh <件数> <シード>` で同じ集合を再生成できる。差が出た`.mesh`は
`/tmp/mesh-oracle-finds.XXXX/`へ残す(捨てると調査もコーパスへの昇格もできない)。

オラクルは`/tmp/mesh-ts-oracle`にキャッシュする。**固定コミット(`cd7273a^`)に紐づくので
worktreeと違って古くならず、消さずに使い回してよい**。復元は初回のみ・1秒未満。
**キャッシュはプロセス間で共有される**——手で壊す実験(ダミー診断の注入など)をすると、
同時に走っている別のセッションが**偽のDIFF**を見る。実際にレビュー中に発生した。
300件で20〜30秒(オラクルのbun起動が支配的。Rust版だけなら1秒)——**環境で変わる**ので、
遅いと感じたら件数を減らすより先に、bunが1件あたり70ms前後で動いているかを見ること。

**導入時点の結果: 2,500件超・シード7種・差0件**。narrowing / `?`と`or` / コレクション /
match・select / 並行処理・defer / ジェネリクス・メソッド・文字列 / **壊れた入力**の
7族を回して一度も食い違わなかった。位置(行:桁)まで含めて比べてもゼロ。
**撤去条件(5)=両実装の並走確認を、凍結コーパスとは独立に再検証した**ことになる。

**語彙を広げるのが主な保守作業**。初版はnarrowingしか作らず800件で0件だった——
**直した直後の領域だけを探索しても意味がない**。その後、handoff が繰り返し記録している
「実装者の検証を通り抜けた不具合は**すべて入力そのものが壊れているケース**だった」に従って
`gen_broken`(未知の型名 / 同名の宣言 / 引数不足 / 値にならない式 / 絞り込み前のunion使用 等
20形)を足し、重みも2倍にしてある。**きれいなプログラムだけ作っても踏めない**。

**0件が続くこと自体は失敗ではない**が、**読まれなくなる**のが失敗。0件が続いたら
「語彙が狭いのでは」と疑って`FAMILIES`へ族を足すこと。

**無人で回す前に潰した4つの穴**(code reviewで発覚。どれも「壊れた環境が、コンパイラの
退行そっくりに見える」形だった):

1. **引数の検証**。`set -e`を使っていないので、生成器が落ちても素通りして
   「対象1件・差0件・OK」という**偽の成功**になっていた(`oracle-hunt.sh abc 123`で再現)。
   件数が0でも同じ——`*.mesh`のglobが展開されず、リテラル文字列1件を検査して両方空で「一致」
2. **キャッシュの完了印**(`.ready`)。`src/cli.ts`の存在だけを見ると、`git archive`は成功したが
   `bun install`が失敗した**中途半端なキャッシュ**を再利用する。その状態ではオラクルが全件
   空を返し、**13件の偽DIFF**になった(標本が小さいと逆に**偽のOK**)
3. **`bun`の確認をキャッシュの有無に関わらず行う**。`bun: command not found`は全件空出力に
   なり、**本物の差分と見分けがつかない**(差1件・FAILという、退行そっくりの出方をしていた)
4. **スモークテスト**。毎回、既知の入力(`y: Nope = 1`)に既知の診断(`unknown-type`)が
   返ることを**比較を始める前に**確かめる。起動できるだけでは足りない

**無人で回すときの自衛(終了コード3=スキップ)**: このスクリプトは**バイナリを読むだけで
リビルドしない**ので、編集途中の作業ツリーで走ると「そのとき たまたま`target/debug`に
あったバイナリ」を正解として扱う。そこで実行前に2つ確かめ、当てはまれば**見送る**:

- `rust/`か`scripts/`に未コミットの変更がある(=編集中)
- バイナリがソースより古い(ビルドし忘れ。`cargo`は`eval "$(mise env -s bash)"`が要るので
  実際に踏んだ事故が「検証の進め方」3.に記録されている)

**失敗(exit 1)ではなくスキップ(exit 3)にするのが要点**。編集中に「差が出た」と騒がれても
意味がないし、「差0件」と報告されるのも嘘になる。呼び出し側が**「見送った」と「差が無かった」を
区別できる**ようにしてある。意図的に汚れたツリーで測りたいときは`ORACLE_HUNT_FORCE=1`。

**終了コードと、自走ループ側の振る舞い**:

| コード | 意味 | ループはどうするか |
|---|---|---|
| 0 | オラクルと一致 | 累計に加算して「累計N件 / 差0件」の1行だけ報告 |
| 1 | `DIFF` または `UNPARSED` | **止めて詳しく報告**(シード・差の内容・保存先) |
| 2 | 環境の問題(bunが無い/オラクルが壊れている/gitが使えない) | **止めてエラーをそのまま報告**。差分ではない |
| 3 | `SKIPPED`(編集中・バイナリが古い) | **累計に加算しない**。「見送り」と1行報告して次の周回へ |

**3を「差0件」と読んではいけない**——測っていないので何も言えていない。累計に足すと
探索量を水増しすることになる。`mise run oracle-hunt`経由だとmiseが3も`ERROR task failed`と
表示するが、**見送りであって失敗ではない**(出力の`SKIPPED:`で判別する)。

**出力の語も分けてある**: ガードの見送りは`SKIPPED:`(exit 3・正常)、生成物がパースできない
のは`UNPARSED`(exit 1・失敗)。当初どちらも`SKIP`と出していて、標準出力を見る運用では
区別できなかった(code reviewで発覚)。

**教訓**: 「0件でした」という報告ほど検証が要る。何も見つからなかったのか、
**見る目が無かったのか**を区別できない道具は、無人で回すと嘘をつく。
道具を壊して`DIFF`が出ることを実際に確かめること(このときはRust側・TS側の両方に
ダミー出力を注入して検証した)。

**次の候補**(未着手): 変数名の一貫リネームで診断不変 / コメント・空行の挿入で診断不変 /
どんな壊れた入力でもコンパイラがpanicしない / **ビルドできたプログラムは実行して期待どおりの
結果を出す**。

最後のものは優先度が上がった。2026-07-28にcodegen側の絞り込みを入れた回のcode reviewで、
**`check`も`build`も通るのに実行結果が誤る**形が見つかったため:

```mesh
fn reset(o: Outer) { o.v = none }
...
if o.v is int { reset(o); print(o.v + 1) }   // → 1 を出す(o.v は none なのに)
```

structが参照値なので、絞り込んだフィールドを**関数呼び出し越しに**書き換えられる
(TS版オラクルも同じ結果なので移植としては忠実。言語レベルの不健全性)。
**`check_implies_build`はこれを捕まえられない**——checkもbuildも通るため。
「その先」を測る性質が要る、という具体的な理由がこれで揃った。
プローブは`tests/parity/narrow-alias-through-call/`。

### オラクルは消えたが「TS版のソース」は git 履歴に残っている(2026-07-28に効いた)

`invalidatePath`の移植で実際に効いたので書いておく。**TS実装は`cd7273a`(TS撤去 段階5)で
削除されただけで、履歴からはいつでも読める**:

```sh
git show cd7273a^:src/checker/narrowing.ts     # 実装
git show cd7273a^:src/checker/statements.ts    # 呼び出し順序
git ls-tree -r --name-only cd7273a^ -- src/    # ファイル一覧
```

**しかも「読める」だけではなく「動かせる」**。`src/`を丸ごと復元すれば TS版CLIがそのまま動く
——`bun src/cli.ts check <file>` で**当時のオラクルに直接問い合わせられる**
(PR #110 のレビューで実際にこれをやり、クロージャ内の代入という際どい形について
「Rust版が保守的すぎるのでは」という疑いを、TS版も同じ位置で同じ診断を出すことを
実測して晴らした)。作業用ディレクトリへ展開して使い、終わったら消す:
...
rm -rf /tmp/ts-oracle   # **必ず消す**(worktreeと同じ扱い)
```

実測で動作確認済み(2026-07-28)。`bun install`は6パッケージ・1秒程度。
`check`以外(`run`/`build`/`fmt`/`test`)も当時のまま使える。

移植漏れを埋めるとき、挙動を推測したり設計し直したりする前に、まずこれを当たること。実際、
`invalidatePath`では「複合代入で誤検知になるのでは」という懸念を抱えて着手したが、TS版の
コメントに**なぜ無効化を型検査の前に置くのか**(順序が逆だと宣言型には代入できるはずの値が
弾かれる)が書いてあり、設計判断そのものが不要になった。

**片付いたもの: `&&`/`||`の右辺に絞り込みが効かない誤検知**(2026-07-28に修正)。

```mesh
if a is Foo && a.value > 0 {   // 直すまでは error[narrow-required](誤検知)
```

`docs/features.md`が「F-6で左の`is`が**右辺の式**とthen節の両方に効く」と書いている形が、
Rust版では効いていなかった。`full_checker::infer_binary`が`&&`/`||`の右辺を絞り込み前の
スコープで検査していたため。原因は典型的なdriftで、`infer_binary`直前のコメントが
「unionは`resolve_scalar_type`でANYに潰れる——ANYは全チェックの安全弁を素通りするので、
絞り込みの有無で結果が変わらない」と説明していたが、**unionをモデル化したmilestone 39以降
この前提は偽**になっていた。前提が消えたのに結論だけが残った形。

**穴の広さ**: `sweep_gen.py`に「絞り込んだ値を条件式の右辺で使う」軸(`cond_*`)を足したところ、
**生成した6件がすべて誤検知だった**。1つの形ではなく、subjectが裸の識別子でもフィールドパス
でも、比較でも算術でも、`&&`でも`||`(De Morgan)でも同じように壊れていた。既存の`CONDS`は
`x is int && 1 == 1`のように右辺が絞り込みと無関係な形しか作らないため、直積を回しても
この族はまるごと出てこなかった。**軸を足した回に穴が見つかり、次の回で埋まった**。

**修正の進め方が今後の型になる**: TS版オラクルを復元して7形すべてが診断ゼロであることを
先に確認し、`expressions.ts`の`inferBinary`(左辺を検査 → `collectFacts` → `pushScope` →
...
codegen単体テストは`full_checker`のゲートを通らないので、**無効化が無ければ即座に
`__iarith`を選んでしまう**——そこを突く回帰テスト(`絞り込んだパスへ代入したら事実を捨てる`)を
置いてある。

**`KNOWN_VIOLATIONS`は8件→3件になった**(その3件は2026-07-29に0件になった。下記の節)。

**この一連で分かった性質検査の働き方**: `full_checker`側の誤検知を直した回に
`KNOWN_VIOLATIONS`が5件→8件へ**増えた**。checkが黙るようになった結果、codegen側の遅れが
露出したからで、これは検査が正しく働いた証拠。次の回でcodegen側を移植して3件へ減った。
**片方を直すともう片方の遅れが見える**——2つの実装を持つ設計では、この検査が
「どちらが遅れているか」を自動で教えてくれる。

### 既知の破れ(`KNOWN_VIOLATIONS`)を0件にした(milestone 68、2026-07-29)

導入時4件 → 8件 → 3件 → **0件**。最後の3件をまとめて埋めた回で、**「記録では原理的に
届かない穴」の実例**が3つそろったので詳しく残す。

**1. 代入文の検査がTS版と構造ごと違っていた**。`check_assign_target`が代入先の種類ごとに
別々の規則を書いていたため、次の4つが食い違っていた:

| 入力 | TS版(オラクル) | Rust版(修正前) | 分類 |
|---|---|---|---|
| `t.n = "s"` | `type-mismatch` | **診断ゼロ** | 検出漏れ(=既知の破れ) |
| `t.n += "s"` | `invalid-operation` | **診断ゼロ** | 検出漏れ |
| `x += "s"` / `xs[0] += "s"` | `invalid-operation` | `type-mismatch` | **違う診断コード** |
| `y /= 0` | `division-by-zero` | 診断ゼロ | 検出漏れ |
| `x = "s"` / `y = 2`(不変) | TS版の文言 | 独自文言・位置もtarget | 文言・位置 |

**この族はparityコーパスに1件も入っていなかった**——`grep -rl immutable-assignment tests/parity`が
空だった。記録との突き合わせでは**永久に見つからない**種類の穴で、オラクルに聞いて初めて分かった。
修正はTS版`statements.ts`と同じく「**代入先の種類で分岐せず`expected`を1つ求めてから共通の
規則を当てる**」形にし、算術は二項演算子と同じ`check_arith`を共有させただけ。

**2〜3. パッケージ修飾のcodegen 2件は「TS版が壊れていた」**ので移植しなかった。
オラクルを復元して実測したところ、どちらも`check`は黙るのに**実行時に落ちる**:

- 非mainパッケージが**自分のトップレベルconst**を無修飾で参照すると、宣言は`lib$LIMIT`・
  参照は`LIMIT`という食い違ったJSになり`LIMIT is not defined`
- `fn (p: lib.Point) show()`は宣言が`__m_Point_show`・呼び出しが`__m_lib$Point_show`になり
  `__m_lib$Point_show is not defined`(TS版`receiverStructName`がレシーバのpkg修飾を無視して
  **現在のパッケージ**で修飾するため)

**「オラクルに聞く」は「オラクルに従う」ではない**、という実例。オラクルが答えるのは
*TS版が何をするか*であって*何が正しいか*ではない。生成JSが実行時に落ちる形では、
そのまま揃えると壊れたものを移植することになる。診断(=移植の判定基準)と
生成コードの正しさは別の軸として見る。

**ついでに直った誤検知**: `fn (p: lib.Point) show()`を宣言しても`p.show()`が
`unknown-field`になっていた(`declare_method`がレシーバのstructを自パッケージの表からしか
引かないためメソッドが登録されない)。TS版が黙る形なので**誤検知**——この移植で最悪の分類。

**実装中に自分で誤検知を作り、テストが捕まえた**のも記録しておく。「解決後の名前に`.`が
あれば他パッケージ」という判定にしたところ、自パッケージのstructも`qualify_struct_name`で
`mathutil.Point`になるため`examples/modules_demo.mesh`が丸ごと`unknown-field`になった。
**判定はレシーバ注釈のpkg修飾で行う**のが正解。handoffの検証手順2'(弾く/特別扱いする
ようにしたものをあらゆる参照経路から使う)がそのまま効いた形。

**`oracle_hunt_gen.py`に代入族(38形)を足した**。語彙を足したときは「0件」が
**空振りでない**ことを確かめること——今回は**修正前のコンパイラをworktreeに用意して
同じシードで回し、9件のDIFFが出る**ことを実測した(修正後は0件)。
...
(`cd7273a^`)なので、**TS版が知っている言語しか判定できない**。本実装(TS版に存在しなかった
新機能)を実装した瞬間、オラクルはそれを`syntax-error`として拒否し、生成物が全部「差分」に
化けて信号が死ぬ。今日組んだ3層のうち、**期限があるのは第3層(オラクル)だけ**:

| 層 | 本実装フェーズでどうなるか |
|---|---|
| 記録(parity/sweep/snapshots) | **そのまま使える**。新機能はコーパスにケースを足して凍結する。
      ただし「凍結時点で既に間違っていたもの」には届かない弱点は変わらず残る |
| **性質検査**(fmt_corpus/check_implies_build) | **そのまま使える。しかも自動で新機能を
      カバーする**——「答えを知らなくても正誤を判定できる」という設計そのものが
      オラクル非依存を意味するため。新機能を足したらコーパスに1件足すだけで守られる。
      **これが今日いちばん価値のある資産** |
| オラクル(oracle-hunt) | **期限切れ**。ポート済み部分集合に対する退行ネットとしては
      残るが、新機能には届かない |

**逆に価値が上がるもの**が2つある:

1. **「ビルドできたプログラムは実行して期待どおりの結果を出す」性質**(まだ未着手)。
   #113で見つけた不健全性(structが参照値なので絞り込みが呼び出し越しに壊れる)のような、
   **checkもbuildも通って実行時だけ誤る**領域。新機能を書けば書くほどこの層の穴が効いてくる
2. **言語カード実証実験**(`docs/card-experiments.md`)。「白紙のAIにカードを渡して実タスクを
   書かせ、詰まったところを穴として拾う」手法。**言語が育つほど価値が上がる**——新機能は
   カードの記述漏れを生みやすく、「AIが書き、人間が読む」という言語の核心コンセプト(P1〜P6)を
   直接検証する。当初のloop engineering段階2の候補だったが、オラクルが使えたので後回しにした

**切り替えの合図は「最初の新機能を実装したとき」**。`scripts/oracle_hunt_gen.py`の`FAMILIES`に
その機能を足そうとして、「オラクルが知らないから足せない」と気づいた時点が分岐点。そこで:

1. `oracle-hunt`は**語彙を凍結**し、ポート済み部分の退行ネットとして残す
   (壊れてはいないので削除しない。ただし新機能の検証には使えないと理解して使う)
2. 自走ループの中身を**性質検査ベースのファジング**へ切り替える——ランダム生成 ×
...
- **checker+codegen milestone 6(モジュール)完了(2026-07-23)**。複数ファイル
  ...
  命名に使用。**当時の意図的なスコープ縮小**: 未export診断・パッケージ誤用診断・パッケージ
  修飾された値参照(呼び出しを伴わない)・パッケージ修飾レシーバ・exportedなconstの
  レジストリ登録は対象外だった(**milestone 49〜51で診断側、milestone 68でcodegen側を
  埋めて全て解消済み**)。新設`modules.rs`5件+`checker.rs`4件+`codegen.rs`7件の
...
4. このアーキテクチャを土台に機能ごとに広げていく(既存21マイルストーンと同じ
   ...
   - ✅ **milestone 24(2026-07-24)でmain関数の形の検査(missing-main/
     invalid-main-signature)を実装**。TS版`checker/modules.ts`の`requireMain`分岐を
     `full_checker.rs`の`check_program`へ移植(full_checkerは単一ファイル専用なので常に
     要求)。診断コード2種を追加、メッセージ・位置ともTS版と完全一致を`mesh check`の
     出力突き合わせで確認。テスト394→399件。詳細はtodo.md参照
   - ✅ **milestone 25(2026-07-24)で演算子の妥当性検査を実装**。TS版
     `expressions.ts`の`inferBinary`+`checkArithOp`・`case "unary"`、
     `statements.ts`の`case "incDec"`を`full_checker.rs`へ移植。診断コード5種
     (invalid-operation/incomparable-types/not-bool/use-is-none/division-by-zero)を
     追加。副産物として文字列リテラル型(`Expr::String`→`Type::Literal`、mut宣言時のみ
     widen)もTS版に合わせた(`1 < "a"`の型表示が`string`ではなく`"a"`になる差の解消)。
     narrowingはスカラースコープではunion型がANYに潰れるため不要と判明。テスト399→412件。
     8演算子ケースをTS版と`mesh check`突き合わせで完全一致確認。詳細はtodo.md参照
   - ✅ **milestone 26(2026-07-24)でargument-count(ユーザー定義関数の引数個数・型)を
     実装**。full_checkerがトップレベル関数を`ANY`ではなく`Type::Fn{params,ret}`
     (新設`fn_signature`)で登録するようにし、`Expr::Call`でcalleeがFn型のとき
     TS版`checkArgsAgainst`相当の個数照合(argument-count)+各引数の型照合(type-mismatch)を
     行う。ローカル変数へ束縛した関数値の呼び出し(`f := add; f(1)`)も照合される
     (TS`checkCallOfValue`と一致)。副次効果で関数呼び出しの結果型が正しく伝播。組み込み関数
     (`builtins.ts`の巨大switch)・pkg修飾・メソッドは対象外(calleeがANY)。ジェネリック関数は
     明示的にANY登録でスキップ(`Type::Fn`で登録するとTS版の`generic-inference-failed`ではなく
     `argument-count`を出す非互換になるため——code reviewで発覚・修正)。テスト412→422件。
     詳細はtodo.md参照
   - ✅ **milestone 27(2026-07-25)で組み込み関数の検査(argument-count/builtin-arg-type)を
     実装**。TS版`builtins.ts`の`inferBuiltinCall`(27組み込みの巨大switch)を
     `full_checker.rs`の`infer_builtin_call`へ移植。`Expr::Call`で裸Identが組み込み名なら先に
     intercept。スカラースコープでは配列/map/channel/関数型がANYに潰れるため、要素型・添字型・
     callback署名の検査は到達せず(コレクションのモデル化とセットで将来対応)、実際に効くのは
     全組み込みのarity・スカラー引数型検査(`len(5)`/`round(3)`/`contains(5,x)`等)・スカラー
     戻り値型。診断コード`builtin-arg-type`追加。テスト422→431件(code reviewで
     文字列引数検査のANY免除漏れ〈false positive〉とreduce戻り値の精密化〈false negative〉を
     発見・修正)。TS版と`mesh check`突き合わせで完全一致、実コレクション使用の正当プログラムで
     誤検知無しも確認。詳細はtodo.md参照
   - ✅ **milestone 28(2026-07-25)でmesh checkもjson structデコーダを合成**。
     全exampleでfull_checkerが無診断か確認したところ`json_decode.mesh`だけ`decodeUser`を
     undefined-nameにする誤検知を出していた——codegen経路は`synthesize_json_decoders`で
     `json struct`の生成デコーダを合成してから処理するのに`run_check`(mesh check)は合成
     していなかった。`run_check`に同じ合成を追加して解消。**これで全single-file example
     でfull_checkerが無診断**になり、`mesh run`/`build`ゲート統合の前提が整った。テスト431→432件。詳細はtodo.md参照
   - ✅ **方向決定(2026-07-25)**: kanayamaが「full_checkerをスカラー卒業」を選択(3択:
     ゲート統合/スカラー卒業/小タスク)。**アーキテクチャ決定(サブフォーク)も確定**——
     型「解決」は既存`checker.rs`(codegen依存ゼロ)を**再利用**、診断「発行」だけfull_checkerに
     **再実装**するハイブリッド(checker.rsのvalidateはResult即失敗で形が違うため。詳細は
     todo.md milestone 29)。この方針で複数milestoneに分けて進める。
   - ✅ **milestone 29(2026-07-25)でstuct卒業の第一歩=名前付きstructリテラル検証**を実装。
     `FullCheckerCtx`に`checker::CheckerCtx`を持たせ`resolve_type_decls`でstruct/unionレジストリを
     構築(再利用)、`infer_struct_lit`でunknown-field/missing-fields/duplicate-field/型不一致を
     診断蓄積形で検査。**milestone 27のinvariant(コレクションは常にANY)は厳密維持**——配列/map/
     union/関数型/pkg修飾は引き続きANY、structのみ解決。診断コード3種追加。テスト432→440件。
     TS版と`mesh check`完全一致・全example回帰なし。詳細はtodo.md参照
   - ✅ **milestone 30(2026-07-25)でstructのフィールドアクセス+メソッド**を実装。
     `check_program`でmethod_tableを構築(resolve_type_annで縮退登録)、`infer_member`で
     フィールド読み(field型/unknown-field/method-not-called)、Callアームでメソッド呼び出し解決
     (引数照合はdefer)。field/method読みとメソッド呼び出しは結合しているので一括実装。
   ...
   - ✅ **milestone 48(2026-07-26)で型宣言の名前衝突**(`already-declared` /
     `builtin-type-redeclared` / `name-conflicts-with-package`)。TS版
     `checker/modules.ts`のcheckPackage冒頭をそのまま移植——組み込み型名 → importエイリアスと
     衝突 → 既に宣言済み、の順に判定し、どれかに当たったらその宣言は**登録しない**。
     実測しないと外す挙動が2つ: (1)**弾かれた宣言は本体も解決しない**
     (`type int = Bogus`に`unknown-type`は出ない)、(2)**報告順が位置順ではない**
     ——名前衝突は全ファイルを1周してから本体解決へ進むので、2行目のalready-declaredが
     1行目のunknown-typeより先に出る。**値側(fn/const/ローカル変数)の
   ...
   - ✅ **milestone 51(2026-07-26)でpkg修飾シンボルの型のモデル化**。**新しい診断コードは
     ...
     **各パッケージが検査の最後に自分の解決済みシンボル表を書き戻す**。可視性(宣言を見れば
     ...
     詳細はtodo.mdの当該項目
   - ✅ **milestone 52(2026-07-27)でstructリテラルの未宣言名に`unknown-type`**。
     型注釈側(milestone 42)を式側へ広げただけで**新しい診断コードは足していない**。
     **実測しないと外す点**: TS版は名前の解決を**値の推論より先**に行うので、
...
- **実装したら必ず一通り検証してからコミットする**(2026-07-19からPRフロー、2026-07-21から
  ...
  6. 次の2つを並行して進める(順不同): `gh pr checks <番号> --watch` でCI green確認、
     ...
     **理由の記載を必須**にしている(見出しだけのコメントは通さない)。判定は
     **各コメントの先頭行だけ**を見る——全文を見ると、マーカーを説明・引用しただけの
     コメント(コードフェンスの中に書いた等)でゲートを通ってしまうため
     (code reviewで実際のすり抜けを再現して判明)
  6''. **自分で検証できる規模なら、エージェントを介さず自分で回す**(2026-07-27追加)。
     下記6'の対策を3つ打った(範囲指定・worktree禁止・観点3つまで)が、**4回目も長時間化した**
     ——`scripts/parity.sh`のレビューで観点を3つに絞ってなお終わらず、結局止めて自分で
     3観点を実機検証したら**数分で済んだ**(しかも自己テストで実バグを1件見つけた)。
     エージェントが効くのは**総当たり**(偽陽性狩りの13位置×4状況、`examples`全件走査)や
     **横断調査**(過去PRのコメント読み)のように、量が多くて機械的な作業。
     「この3点を確かめる」程度なら自分でやるほうが速いし確実。
  6'. **レビュー用エージェントには「調べる範囲」と「調べ方」を必ず指定する**(2026-07-26追加。
     このセッションで2回、エージェントが**1時間以上ハングして手が止まった**)。
     ...
     - **件数やclippyの検証に`git worktree`+フルビルドを使わせない**。
       ...
       `cargo test`を1回流せば足りる**と明示する
     - **エージェントを起動したら、同時に「目覚まし」を仕掛ける**(2026-07-27追加。
       これが唯一の**機械的な**対策):

           Bash(run_in_background: true) で
           scripts/agent-timeout.sh 10 <エージェントID...>

       `run_in_background` のコマンドは**終了時に必ず通知が来る**ので、10分後に1回
       割り込みが入る。これが無いと**そもそも気づけない**——`TaskList`にエージェントは出ず
       経過時間も見えないので、待っている側からは「まだ考えている」と区別がつかない。
       4回とも人間に指摘されて初めて気づいた。エージェントが先に終わったら目覚ましは
       `TaskStop`で止めてよい(放っておいても1回鳴るだけ)
     - **止まったら`TaskStop`で止めてよい**。5観点のうち1つが落ちても、他が実機検証して
       いれば網羅性は保てる(実際、最重要の偽陽性はどのセッションでも別のエージェントが
       見つけている)。**`TaskList`には出ないので、起動時のエージェントIDを渡す**
     - **エージェントが作ったブランチ/worktreeは残る**。マージ後に
       `git worktree list` と `git branch` を必ず確認する(このセッションでは
       比較用ブランチが5本残っていた)
...
- **「偽陽性狩り」専任エージェントを1体立てる**(2026-07-26追加。milestone 47・48・49で有効性を確認)。
  この移植で最悪の不具合は**Rust側だけに出る診断**(TS版が出さないものを出す)で、
  milestone 47・48は**実装者の検証を通り抜けて**このクラスの不具合をcode reviewに
  見つけられている。汎用の「バグを探して」では見つからず、**目的を1つに絞った専任
  エージェント**が効いた:
  - 指示は「**TS版が出さない診断をRust版が出す入力を探せ**。それだけを報告せよ」と単一目的にする
  - **両CLIを実際に走らせる手順を渡す**(`eval "$(mise env -s bash)"`が毎コマンド必要・
    TS版は`bun src/cli.ts check`・Rust版は`rust/target/debug/mesh check`・
    **リポジトリルートから実行**しないと相対パスが壊れる)
  - **攻撃面を列挙して渡す**: その回で新しく弾く/特別扱いするようにしたものを、
    参照位置(引数・戻り値・structフィールド・ローカル注釈・配列/map/chanの中・
    type alias本体・structリテラル名・レシーバ・`is`/matchのパターン)へ総当たりさせる。
    加えて組み込みパッケージ・未使用/未importのimport・shadow・重複宣言・
    空パッケージ・構文エラー・`mesh test`経路・ダイヤモンドimport
  - milestone 49ではこの形で**13の参照位置×4状況**を回して偽陽性0件を確認できた
    (直近3回は出荷しかけていた)
