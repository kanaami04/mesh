...
4. このアーキテクチャを土台に機能ごとに広げていく(既存21マイルストーンと同じ
   ...
   - 残る候補: **milestone 30以降でstruct卒業を継続**——フィールドアクセス(`u.name`)の型解決+
     unknown-field・メソッド(method_table+method-not-called)・判別可能union構築・pkg修飾struct・
     配列/map型のモデル化(collection builtin要素型検査とセット)。その後: `mesh run`/`build`ゲート
     統合+RustCLI整備・generics推論(`generic-inference-failed`)・parser/lexerのDiagnosticCode統合
