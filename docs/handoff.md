...
**次に片付けたいもの その1(2026-07-28に実測で発覚、未修正)**: **`invalidatePath`の移植漏れ**。
TS版は絞り込んだパスへ代入したら事実を捨てていた(`narrowing.ts`/`statements.ts`のassignケース)が、
`full_checker.rs`に相当する処理が無い。`docs/features.md`が「フィールドへの代入は古い絞り込みを
無効化してから型検査するので健全性は保つ」と書いている安全性が**実装されていない**。
現状は検出漏れに留まっているが、**codegen側のフィールドパス絞り込みを入れる前提条件**になる
(上の`KNOWN_VIOLATIONS`の顛末を参照——順番を逆にするとミスコンパイルになる)。
プローブは`tests/parity/narrow-then-assign-field/`。

**設計の注意**: 素直に「代入したら無効化」を入れると`o.inner.v += 1`のような複合代入で
新しい誤検知を生みうる。**無効化の前に検査するのか後にするのかで、どちらかが誤検知になる**——
オラクルが無い今、ここは実測ではなく**設計判断**が要る領域。急いで入れないこと。

**次に片付けたいもの その2(同日発覚、未修正)**: **`&&`/`||`の右辺に絞り込みが効かない誤検知**。
