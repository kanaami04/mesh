...
4. このアーキテクチャを土台に機能ごとに広げていく(既存21マイルストーンと同じ
   ...
   - 残る候補: struct/フィールド関連の診断(struct/配列/map/channel等のモデル化=スカラー
     スコープの卒業とセット)・`mesh run`/`build`へのゲート統合・generics推論
     (`generic-inference-failed`)・parser.rs/lexer.rsの既存コードをDiagnosticCodeへ統合
