...
1. ~~**性質検査 第3弾**~~ ✅ **2026-07-29に`build ⇒ run`として実装した**(下記
   「性質検査 第3弾」節)。「期待どおりの結果」は答えが必要になるので、**「コンパイラの
   バグで落ちない」へ言い換えて**オラクル非依存にした。**残っているのは結果の正しさ**
   ——`tests/parity/narrow-alias-through-call/`の不健全性はまだどの層も捕まえない
2. **性質検査 第4弾(一斉リネームで不変)** ——**その準備で言語の穴が2つ出たので先に塞いだ**
   (下記「`__`接頭辞を予約した」節)。リネーム生成器を書く前に「危ない名前」を手で試したところ、
   `__m`/`__idx`のような名前が**コンパイラの内部名を隠して壊れる**ことが分かった。
   本体(リネーム生成器 + 診断・実行結果の突き合わせ)はまだ未着手。範囲は
   **ローカル+引数+トップレベルのfn/const名**(`main`とstructフィールドは除く。
   フィールド名は`print`出力とjsonのキーに出るので実行結果の比較ができなくなる)
3. **コメント・空行の挿入で診断不変** / **どんな壊れた入力でもコンパイラがpanicしない**
4. **最初の新機能に着手する**(= オラクルを捨てる決断とセット。下記「loop engineeringの
   見取り図が変わる時期」)
5. **言語カード実証実験 第12回**(大規模複合タスク)
...
2''. **そのマイルストーンが偽にしたコメントを機械的に探す**。**milestone 46〜49で4回連続、
   ...
   **4種類の検索語を回す**: (a)移植した診断名、(b)その回で**撤去した仕組みの名前**
   ...
   (d)**触ったランタイム内部の名前**(`rust/embedded/runtime.ts`の`__panic`等。2026-07-29追加
   ——(b)のRust向けパターン`fn name`ではJSの`const __panic = (e) =>`を拾えず、
   `__panic`の表示を変えた回に`drift.sh`が「差分が無い」と報告した)。
...
- `rust/tests/build_implies_run.rs`「**診断ゼロでビルドできたプログラムを実行しても、
  コンパイラのバグでは落ちない**」(2026-07-29。下記「性質検査 第3弾」節が詳細)。
  `check⇒build`の次の層で、**実行までを見る**。判定はランタイムが出す`internal error:`の
  印だけなので、プログラムの出力を知らなくてよい。現在58件を実行して0件。
...
**2026-07-31に「生成JSの突き合わせ」を足した。** それまでoracle-huntは`check`しか通しておらず、
**診断だけ**を比べていた。しかしmilestone 66で見つかった「json structのエンコーダ合成が
丸ごと未移植」は**診断が一致したまま生成JSだけが違う**形で、この軸でしか見えない。
コーパスに対しては`rust/tests/codegen_snapshot.rs`が生成JSを見ているが、あれは
**記録との比較**(TS撤去時に凍結したもの。`scripts/parity.sh`の冒頭コメント参照)で
**オラクルとは比べていない**。つまり「ランダムな入力 × オラクル × 生成JS」は
**誰も見ていなかった**——そこが埋まった。

- **preludeは比べない**。Rust版の`rust/embedded/runtime.ts`は撤去後も変更が入る
  (2026-07-29に`__panic`の表示を分けた)のに対し、TS版オラクルは`cd7273a^`で凍結されている
  ので**必ず食い違う**。`// ===== end runtime =====`以降の**本体だけ**を比べる
- **診断ゼロのケースだけが対象**(診断が出るプログラムはビルドされない)。そのため
  **「生成JSも比べた件数」を必ず出す**——300件中175件、のように。出さないと「差0件」が
  「比べた結果0件」なのか「1件も比べていない」のか読み手に区別できない
- **偽のDIFFを1つ踏んだ**: TS版の`build -o /dev/stdout`は「wrote /dev/stdout」という
  ステータス行を本文に混ぜるので、30件中18件が全部差分に見えた。一時ファイル経由にして解消
- **道具が働くことを実測した**: `codegen.rs`の不変束縛を`const`→`let`に変える
  (**診断は一切変わらない**)と、30件中15件でDIFFが出る。復元後は0件
- **導入時の結果: 既存の語彙では900件・3シードすべて差0件**(生成JSまで含めて)。
  つまり**いま探索できている範囲の移植は生成コードまで忠実**で、価値は
  **語彙を広げること**の側にある(下記)

**生成器がまだ作れないもの**(オラクルが消える前に埋める価値がある順):

1. **パッケージ(複数ファイル)**。生成器は`.mesh`を1本しか書かない。一方
   milestone 68で見つけた4件のうち**3件がパッケージ絡み**だった
2. **`json struct`**。小さな宣言から大量のコードが合成される=行あたりのバグ密度が高いのに
   ランダム生成が一度も届いていない
3. **`error type`(構造化エラー)**
...
どんな壊れた入力でもコンパイラがpanicしない。

「**ビルドできたプログラムは実行して期待どおりの結果を出す**」は 2026-07-29 に
**`build ⇒ run` として着手済み**(下記「性質検査 第3弾」節)。ただし埋まったのは
「実行できること」までで、**結果の正しさ**は下記の不健全性を含めて未着手のまま。

もともとこの候補の優先度が上がったのは、2026-07-28にcodegen側の絞り込みを入れた回の
code reviewで、**`check`も`build`も通るのに実行結果が誤る**形が見つかったためだった:
...
**`build_implies_run`でもまだ捕まらない**(実行はできて、値だけが誤るため)。
残っているのはこの層。

### 性質検査 第3弾: `build ⇒ run`(2026-07-29)

`rust/tests/build_implies_run.rs`「**診断ゼロでビルドできたプログラムを実行しても、
コンパイラのバグでは落ちない**」。`check_implies_build`が見ていたのは**コード生成が
成功すること**までで、生成されたJSが実行できるかは別だった——実際そこに穴があり、
milestone 68のcode reviewが**人力で**見つけている(非mainパッケージのconst参照が
`check`も`build`も通って実行時に`x is not defined`)。

**「期待どおりの結果」を測ろうとすると答えが必要になる**ので、そのままでは
オラクル非依存にならない(記録にすれば`--update`の抜け道も戻る)。そこで判定を
言い換えた:

| 実行時に起きたこと | 誰のせいか | 判定 |
|---|---|---|
| `__Panic`(範囲外アクセス・整数オーバーフロー等) | ユーザーのプログラム(仕様内) | 通す |
| **ホストの限界**(スタック溢れ) | ユーザーのプログラム(再帰が深い) | 通す |
| それ以外の素のJSエラー(`ReferenceError`等) | **コンパイラ**(生成JSが壊れている) | 落とす |

標準ライブラリの失敗経路はすべて`error`値へ変換している(`io$readFile`/`json$parse`/
`http$listen`)ので、**その3つを除けば`__panic`へ届くのはコンパイラのバグだけ**。
そこで**ランタイム側で表示を分けた**(`panic:` / `internal error:`)。検査は
「`internal error:`が出ないこと」だけを見るので、プログラムが何を出力するかを知らなくてよい。
現在**58件を実行して0件**(残り110件は診断が出るので性質の前提外)。

**「ホストの限界」を別枠にするのはcode reviewの指摘で入った**。当初は「`__Panic`以外は
生成JSの壊れ」としていたが、この主張は**不完全**だった——標準ライブラリの失敗経路は
確かに全部`error`値になるが、**JSエンジン自身の限界はそのどれも通らない**。素朴な判定だと
ユーザー自身の深い再帰が`internal error: Maximum call stack size exceeded`+
「これはMeshコンパイラのバグです」として報告される(実測して確認)。
**判定はスタック溢れだけに絞る**——RangeError全体を仕様内へ寄せると、本物のバグが
RangeErrorの形で出たときに見逃す。

**この回で得た教訓が2つある**:

1. **コーパスに形を置くまでが仕事**。導入直後にcodegenを壊しても検査は落ちなかった——
   非mainパッケージのconstを`is`で絞り込む形がコーパスに無かったため。
   `tests/parity/pkg-const-runtime/`を足してから壊すと`internal error: maybe is not
   defined`で落ちた。milestone 65・PR #112の「性質検査に守らせたい形は、コーパスに
   置くまでが仕事」がそのまま再現した。
2. **土台の検査は「印が出る方向」を測らないと守りにならない**。判定の土台
   (ランタイムの表示し分け)を守るテストを最初「仕様内のpanicに`internal error:`が
   付かないこと」だけで書いたが、`internal = false`と壊しても**両方のテストが素通り**した
   ——仕様内のpanicは正しいままだから。ランタイムの`__panic`へ直接
   `new Error(...)`と`new __Panic(...)`を渡して**両方向**を固定して解決した。
   PRELUDEは`--emit-js`の出力から取り出す(**出荷する本物を測る**——書き写すと写しの側だけ
   直して安心する形になる)。

**壊して確かめた記録**: (a) `Expr::Is`のマングルを素の名前へ戻す → 性質検査が
`internal error: maybe is not defined`で落ちる。(b) ランタイムの印を消す → 土台の検査が落ちる。
どちらも復元済み。

**code reviewが見つけた3件目: 同じ判定を要する経路が3つあった**。`__panic`だけ分けていたが、
`__httpDispatch`(リクエストごとの障害分離)と`__runTests`(テスト1件ごと)も同じ問いを
必要としていた。PR #113の「**同じ構文を扱う別経路を片方だけ直す**」がまた再現した形で、
milestone 68の`ident_js_name`と同じく**判定を1箇所(`__isCompilerBug`/`__blame`)に閉じて**
解決した。**「3経路が共通の判定を通っていること」自体を検査する**テストも置いてある
——閉じたつもりで1つ漏れる形を機械で防ぐため。

### `__`接頭辞を予約した(2026-07-29)——性質検査 第4弾の準備で出た言語の穴

**「一斉リネームで診断・実行結果が不変」の生成器を書く前に、危ない名前を手で試した**ら
言語レベルの穴が2つ出たので、先にそこを塞いだ回。**リネームの性質検査そのものは未着手**。

生成JSは一時変数とランタイム関数を`__`接頭辞に置いている(`__m`/`__v`/`__d0`/`__idx`/
`__panic`…)。ユーザーが同じ名前を宣言すると**フラットなJSスコープで内部名を隠す**:

```mesh
__m := 100
r := match v { int => __m, none => 0 }   // → 100 ではなく **subjectの値** を返す
__idx := 42
print(xs[0])                             // → 42 is not a function
```

1つ目が特に悪い——**checkもbuildも通って実行結果だけが誤る**。
**どちらもTS版オラクルで同じ結果**(`7`を返す/同じ実行時失敗)なので、移植の問題ではなく
**最初からある言語の穴**だった。

**討議して「`__`で始まる名前の宣言を禁止する」を採用した**(kanayamaと確認)。判断の根拠:

- **Meshでは`__`に意味が無い**。Pythonの名前マングリングやJSの`#`と違い、ただの識別子。
  可視性は`export`の有無で決まる設計なので「`__`は内部向け」という第2の慣習は入る余地が無く、
  P1(書き方は一つ)にも反する。**ユーザーが失うものは無い**
- 逆にコンパイラは「ここは自分のもの」と言える名前空間を得る——**一時変数を1つ増やすたびに
  誤コンパイルの種が増える**状態を終わらせられる
- フィールド名`__proto__`を既に個別に拒否していた(prototype汚染対策)ので、**場当たりを
  規則に一般化する**方向でもある

実装は`declare()`の1箇所+診断は既存の`reserved-word`を流用(**新しい診断コードは増えない**)。
`__`で始まる名前を書いたプログラムはTS版オラクルと食い違うが、`oracle_hunt_gen.py`は
`__`名を作らないので探索には影響しない。

**parityを回して初めて誤検知10件に気づいた**のが実地の教訓。`json struct`の合成デコーダは
`__ef_*`のような名前を**10種類**使っており(`json_decode.rs`)、それは`__`を予約した
**目的そのもの**なので弾いてはいけない。合成物は「以降のcheck/codegenを無改造で流用する」
ために普通の`FnDecl`として`program.fns`へ積まれる設計なので**ASTからは区別できない**
——`check_package`がjson structから決まる名前(`decode{Name}`/`encode{Name}`)で判定し、
その本体を検査中だけ`ctx.in_synthesized_fn`を立てて免除する形にした。回帰テストつき。

**同じ回でもう1件出た: `_ = expr` が壊れたJSを生成していた**。`__`の予約が
**単一のアンダースコア(ブランク識別子)を巻き込んでいない**ことを確認していて見つかった。
`_`はどこにも宣言されないのにcodegenが素直に`_ = ...`と出しており、`mesh check`は通るのに
実行時`_ is not defined`で落ちる——**`build ⇒ run`の破れ**(第3弾の性質検査が
`internal error:`として正しく分類した)。右辺を式文として出す形に直し、**副作用は保ったまま
値だけ捨てる**ようにした。経路は文(`gen_stmt`)とforのinit/post(`gen_simple_stmt`)の
**2つ**で、いつもの「片方だけ直す」を避けて両方直してある。

**その修正が今度は逆向きの穴を作りかけた**(自己レビュー中に実測して発見)。`_`の近道を
複合代入にも当てていたため、`_ += 1`が**診断ゼロで黙って何もしない**ようになっていた
——TS版は同じ入力で実行時に`_ is not defined`と落ちるので、**「うるさい失敗」を
「静かな無動作」に変えていた**(このリポジトリが退行と呼ぶ形そのもの)。
`_ += 1`は`_`を**読む**形なので、`print(_)`が`undefined: '_'`になるのと同じ規則を当てて
解決した(近道は素の代入だけに限定)。**TS版はここを一律に飛ばしている**ので揃えていない
——揃えると検出漏れを移植することになる。診断コードは既存の`undefined-name`。
**「捨てる」と「読む」を区別するのが要点**で、`_++`/`_--`も読む側。

**さらにcode reviewが3つ目の経路を見つけた**: トップレベルの `_ := expr`。
codegenが`const _ = ...`を出しており、2つ書くと`declared_consts`のガードに当たって
**`mesh check`は黙るのに`mesh build`が落ちる**(`check ⇒ build`の破れ)。
ローカルの`ShortVarDecl`は正しく捨てていたので、**同じ構文を扱う経路のうち1つだけが
取り残されていた**——PR #113の教訓がまた出た形。捨てる経路は**4つ**(ローカル宣言 /
文の代入 / forのinit・post / トップレベル)。
**コーパスに置いてから壊すと`check_implies_build`が落ちる**ことも実測した
(置く前は空振りしていた——「守らせたい形はコーパスに置くまでが仕事」)。
