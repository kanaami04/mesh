...
4. このアーキテクチャを土台に機能ごとに広げていく(既存21マイルストーンと同じ
   ...
   - 残る候補(次はより大きなアーキテクチャの選択): (a)`mesh run`/`build`へのfull_checker
     ゲート統合(単一ファイルは前提が整った。RustCLIは現状checkと--emit-jsのみの疎通確認用
     なのでCLI整備とセット)・(b)full_checkerをスカラースコープから卒業させてstruct/配列/map/
     union等をモデル化(struct/フィールド診断・コレクション要素型検査の前提)・(c)generics推論
     (`generic-inference-failed`)・(d)parser.rs/lexer.rsの既存コードをDiagnosticCodeへ統合。
     (a)(b)は複数milestoneにまたがる大きめの方向なので着手前にkanayamaと相談する
