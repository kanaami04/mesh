...
2. **性質検査 第4弾の候補**: 変数名の一貫リネームで診断・実行結果が不変 /
   コメント・空行の挿入で診断不変 / どんな壊れた入力でもコンパイラがpanicしない。
   1つ目は絞り込みのパスキー・constのマングル・codegenの一時変数(`__v`/`__m`/`__d0`)という
   **名前ベースの機構**を突けるので、milestone 68の穴と同じ族に効く
3. **最初の新機能に着手する**(= オラクルを捨てる決断とセット。下記「loop engineeringの
   見取り図が変わる時期」)
4. **言語カード実証実験 第12回**(大規模複合タスク)
