...
- `rust/tests/check_implies_build.rs`「**`mesh check` が診断ゼロならコード生成も成功する**」。
  ...
  **導入時点で4件破れていた**(`KNOWN_VIOLATIONS`に理由つきで登録。リストは両方向に効くので、
  直したのに消し忘れることはできない)。うち3件はcodegen側の既知の「未対応」だが、
  **1件はこの検査で初めて見つかった移植漏れ**——入れ子のフィールドパス
  (`if o.inner.v is int { o.inner.v + 1 }`)の絞り込みが`checker.rs`に無く、
  `invalid operation: int | none + int`になる(言語機能としてはF-6で実装済み。
  `full_checker.rs`は正しく絞り込む)
...
**あわせて片付けたいもの**: 上で見つかった`checker.rs`のフィールドパス絞り込みの移植漏れ。
`KNOWN_VIOLATIONS`から1件消せるだけでなく、**`mesh check`の約束を守る**ための修正になる。
