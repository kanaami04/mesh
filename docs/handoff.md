...
**撤去条件(1)(2)を満たしたので、いまは `docs/ts-removal-plan.md` の段取りを進めている。**
段階1(CIで両実装を並走)・段階2(`include_str!` 3ファイルをRust側へ複製)は実施済み。
**`src/`を消してもRust版はビルドでき、全テストが通る**(worktreeで実測)。
残るTS依存はオラクル(parity/sweep)としてだけ。段階3(TSテスト資産の移植)も完了
——領域別に数えるとRust側の方が多いものがほとんどで、**丸ごと欠けていた観点は2つだけ**だった
(`mesh/http`の実行時テスト・`mesh fmt`のコーパス全体での性質テスト)。
段階4(playgroundのwasm化)も完了——`wasm-bindgen`を入れず手書きABIで済ませたので
**依存クレート0は維持している**。**残るのは段階5(撤去の実行)だけ**。
