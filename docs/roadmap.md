...
- 改行はNewlineトークン化済み(2026-08-15。複数行ソースが読める)。**CRLF(`\r\n`)の扱いが仕様1章に未定義**(現状 `\r` はE0116)——許可+正規化か拒否かを決めて仕様に追記する。`;`→E0110、非ASCII識別子→E0103、`/*`→E0102 は引き続き暫定E0116(src/lexer.rsのErrorCode docに一覧)。文終端/継続の判定(L-19/L-20/L-21)はパーサ着手時に配置を設計する
