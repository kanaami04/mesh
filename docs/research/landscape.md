...
Musubiの立ち位置を定めるための調査。結論: **「FE+BE両対応」の実現方式は業界的に「JSへコンパイル」か「WASMへコンパイル」の実質二択**であり、各言語はその路線上で差別化している。
...
- **Musubiとの差別化ポイント**: MoonBitは「クラウド・エッジ計算向け+汎用」でありUIは主戦場ではない。Musubiは**UI構文を言語に組み込み、フルスタックWebアプリに全振り**する。また日本語話者の作者による、読みやすさ最優先の設計。
- ユーザー所感: 近いが想像しているものではない(→どこが違うかをPhase 1で言語化するとMusubiの輪郭が明確になる)。
...
- **Musubiへの示唆**: ①Tokioチームですら「ブラウザ=WASMよりJS変換」を選んだ(MusubiのJSターゲット決定を裏付ける)。②「FE/BE境界のAPIボイラープレート排除」はMusubiのフルスタック統合(Phase 7)の重要な参考例。③ただしTopcoatは既存言語Rustの上のフレームワークであり、言語ごと設計できるMusubiの方が構文の自由度は高い。
...
- **Musubiへの示唆**: 「1言語・複数ターゲット・FE/BEコード共有」が実際に成立する証拠。またターゲット間で無理に意味論を統一しない割り切り方が参考になる。

## その他の参考(いいとこ取りの元ネタ)

- **TypeScript**: 型システムの表現力とJSトランスパイルの成功例。ただし`any`と歴史的経緯が「抜け穴」。ソースマップ・エディタ体験は手本。
- **Elm**: FE専用だが「親切なエラーメッセージ」の金字塔。MusubiのエラーメッセージのUX目標。ただしFE専用に閉じたことがエコシステム的な限界にもなった(→BE軸を持つMusubiの逆張り根拠)。
- **Dart/Flutter**: 「言語+UIフレームワーク一体開発」の成功例。UIを言語と共進化させる価値の証拠。
- **ReScript**: JSターゲットのML系。生成JSの読みやすさを売りにしている(生成コードが読める=デバッグしやすい、はMusubiも重視したい)。
