---
name: git-worktree
description: 別のコミット/ブランチの状態を、いまの作業ツリーを壊さずに用意する(比較・隔離)。**作ったworktreeを必ず消すまで**が手順。「変更前後の挙動を比べたい」「このバグはこのPRが入れたのか確かめたい」「別ブランチを同時にチェックアウトしたい」ときや、その目的で`git stash`/`git checkout`を使おうとしたとき、`git worktree add`を打つときに使う。
---
...
- **`git stash` で「変更を一時的に外す」のは禁止**(2026-07-26と07-27に事故。
  **2026-07-27から`.claude/hooks/block-git-stash.sh`が機械的に拒否する**——
  2回目は「ここに禁止と事故内容が書いてある」状態で踏んだので、文書ではなくフックにした。
  読み取り専用の`git stash list`/`show`は通る): 作業ツリーが
