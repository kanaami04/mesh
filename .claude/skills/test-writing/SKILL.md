...
- **IDの先行予約**(2026-08-15ユーザー承認): 仕様のテストIDは、対応する実装が未着手の場合に限り、仕様の対応表に「(○○サイクルで実装)」の注記つきで先行予約できる(実例: `comment-before-continuation`=終端挿入サイクル)。回収先を**roadmapの該当タスク行にも書いて二重追跡**にする。注記のない未実装IDは「仕様に書いたのにテストが無い状態」の違反であり、impl-review・drift-checkの検出対象。
