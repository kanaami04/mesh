...
- **1テスト=1検証項目、assertは原則1つ**(2026-08-15ユーザー決定)。複数入力を検証したいときは関数を分ける。同一検証項目の複数入力は代表1入力に統合する(実例: `_tmp`+`_1` → `_tmp1`)。`expect`(Okである前提の確認)はassert数に数えない。パラメータ化テストcrate(rstest等)は導入しない——関数分割なら「テスト名=仕様文」の価値が保てる。
