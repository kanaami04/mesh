#!/bin/bash
# PreToolUse(Bash) hook: `gh pr merge` の前に、そのPRへコードレビューの結果
# （`### Code review` 見出しのコメント）が実際に投稿されているかを確認する。
# 無ければ deny して理由を返す。
# それ以外のコマンドは素通り（exit 0 + 出力なし = allow 判定に委ねる）。
#
# **レビュー自体は push の前に回す**（2026-07-28にmerge直前から移動。
# enforce-review-before-push.sh がそちらのゲート）。このフックが見ているのは
# 「push前に回したレビューの結果がPRに記録されたか」であって、レビューを2回
# 回させるためのものではない。
#
...
for n in "${!seen[@]}"; do
  ...
  hook_deny "PR #$n にはまだコードレビューの結果が投稿されていません。**push の前に回したレビュー**の結果を \`### Code review\` という見出しのコメントとしてPRに残してください（例: \`gh pr comment $n --body-file <file>\`）。PR単位でレビューをやり直したい場合は \`/code-review $n --comment\` も使えます（指摘が見つかった場合は対応してから再度レビューを通してください）。レビューが不要だと判断した場合は、その旨と理由を \`### Code review skipped: <理由>\` という見出しのコメントで残してください（見出しは**コメントの先頭行**に置いてください。前置きの文章を挟むと認識されません）。"
  exit 0
done
